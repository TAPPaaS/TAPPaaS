...
#
# Proves a service published to the internet via network:proxy is reachable
...
[[ -n "${DEF_DOMAIN}" && "${DEF_DOMAIN}" != CHANGE* ]] \
    || { warn "SKIP: no default-environment domain set (set domains.primary in the default environment file)"; exit 0; }
