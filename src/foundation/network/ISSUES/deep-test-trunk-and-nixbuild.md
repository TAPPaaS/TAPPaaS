...
and network:proxy per-service now use it instead of `dns-manager`. Verified live:
...
Root cause: Sprint 4's `acme-setup.sh` (and `network:proxy` per-service) register
...
used by acme-setup §6 / network:proxy per-service is the wrong mechanism for this
