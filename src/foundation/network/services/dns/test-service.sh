...
info "network:dns test-service for module: ${BL}${MODULE}${CL}"
...
if [[ "${FIREWALL_TYPE}" == "NONE" ]]; then
    ...
    info "${GN}network:dns test-service completed for ${MODULE} (skipped)${CL}"
    exit 0
fi
...
info "${GN}network:dns test-service passed for ${MODULE} (${FQDN})${CL}"
exit 0
