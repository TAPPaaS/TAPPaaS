...
info "network:rules test-service for module: ${BL}${MODULE}${CL}"
...
if [[ "${FIREWALL_TYPE}" == "NONE" ]]; then
    ...
    info "${GN}network:rules test-service completed for ${MODULE} (skipped)${CL}"
    exit 0
fi
...
if rules-manager verify-rules "${MODULE}" ${DEEP_FLAG} \
        ...
        --no-ssl-verify; then
    info "${GN}network:rules test-service passed for ${MODULE}${CL}"
    exit 0
else
    error "${RD}network:rules test-service detected drift for ${MODULE}${CL}"
    exit 1
fi
