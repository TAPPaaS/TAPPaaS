...
- **`./test.sh`** (fast) — the module test.sh; validates the template config JSONs
  ...
  parses the service scripts; flags the NixOS and Debian test stubs.
...
- The per-OS **service tests** are owned by the *consumer* modules and invoked by
  `test-module.sh` for `templates:nixos` / `templates:debian` / `templates:windows` dependents:
...
- `services/debian/test-service.sh`: a STUB, the same shape. It exists so a Debian dependent is reported as tested rather than "no test-service.sh"; it asserts nothing.
...
- None — neither service test has a deep tier. The standard windows tests are themselves live (SSH + PowerShell against the VM). Unverified: NixOS and Debian baseline correctness entirely (both tests are stubs); for Windows, anything beyond the four baseline checks (patch level, network/zone wiring, installed roles/software, RDP reachability through the firewall rather than just the registry flag).
...
- ⚠️ `services/nixos/test-service.sh` and `services/debian/test-service.sh` are
  still PLACEHOLDERS with zero assertions ("no tests implemented yet") — the NixOS
  and Debian baselines are effectively UNTESTED despite a green exit. Implementing
  them is the remaining open item.
