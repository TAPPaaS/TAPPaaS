...
- **Test 9z: every tabletop suite under `scripts/test/`** — swept, not listed: a file added there runs the day it lands. Includes `test-repo-sync-stash.sh` (#572 — a sync's auto-stash is put back afterwards; one that no longer applies is kept and counted, never silently dropped).
