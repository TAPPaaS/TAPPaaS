...
- **Test 9z: every tabletop suite under `scripts/test/`** — swept, not listed: a file added there runs the day it lands. Includes `test-repo-sync-stash.sh` (#572 — a sync's auto-stash is put back afterwards; one that no longer applies is kept and counted, never silently dropped) and `test-migration-runner.sh` (ADR-025 / #652 — order, ledger, `--list`/`--check` write nothing, a failure stops the run before the rebuild, backups and their retention, `--rerun`, `--baseline`). A migration's own fixture test joins this sweep by existing (ADR-025 D7).
