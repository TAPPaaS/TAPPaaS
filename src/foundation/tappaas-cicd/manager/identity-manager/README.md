...
The **Identity domain** manager. It owns the Organization → Group → User hierarchy
...
Config state lives under `config/identities/` (default
...
placeholders, seeded by `identity-manager bootstrap`.
...
This manager exposes one compiled CLI (`identity-manager`) plus one bash helper
(`validate-people.sh`).

### `identity-manager` — bootstrap, read, CRUD + reconcile

```
identity-manager bootstrap --org O --user U --email E [--minimal-org DIR] [--force] [--skip-validate] [--config-dir DIR]
identity-manager reconcile  [--apply]  [--config-dir DIR]   (alias: sync, deprecated)
identity-manager validate                            [--config-dir DIR]
identity-manager <kind> list   [--json] [--deep]     [--config-dir DIR]
identity-manager <kind> show   <name> [--json]       [--config-dir DIR]   (alias: get, deprecated)
identity-manager <kind> add    <name> [field flags]  [--force] [--config-dir DIR]
identity-manager <kind> modify <name> [field flags]  [--config-dir DIR]
identity-manager <kind> delete <name> [--force]      [--config-dir DIR]
identity-manager -h | --help
```
...
`config/identities/<dir>/<name>.json` **and then push it to the identity service**
...
```
identity-manager <kind> add|modify|delete ...                  # write + push
identity-manager <kind> add|modify|delete ... --no-reconcile   # write only (stage)
identity-manager reconcile [--apply]                            # preview / push
```
...
3. **Then a full reconcile**, not just this entity's actions: `config/identities` is
...
`identity-manager reconcile --apply`, not to redo the edit.
...
```bash
identity-manager user list
identity-manager org show foo-company
identity-manager role add editor --displayName "Editor"
identity-manager user add jan --email jan@foo.nl --roles user --groups foo__users
identity-manager user modify jan --add-roles admin --remove-groups foo__users
identity-manager group delete foo__users          # refused if any user is a member
identity-manager role delete editor --force        # delete despite references
identity-manager user add jan --email jan@foo.nl --no-reconcile   # stage, do not push
identity-manager reconcile                          # preview the plan (default)
identity-manager reconcile --apply                  # apply to the identity service
```

### `identity-manager bootstrap` — bootstrap a minimal org

Copies `minimal-org/` into `config/identities/`, substituting the placeholders
(`__ORG__` / `__USER__` / `__EMAIL__` / `__ROOT_EMAIL__`) in both filenames and
contents. Pure file bootstrap — makes no identity calls (run
`identity-manager reconcile --apply` afterwards). Native since the
...
```
identity-manager bootstrap --org <slug> --user <slug> --email <email>
              [--config-dir <path>]    # dest (default $TAPPAAS_CONFIG/identities)
...
the bootstrap once `config/identities/` is populated. The result is validated for
reference integrity (the `identity-manager validate` gate); exit 0 = success,
1 = error.

```bash
identity-manager bootstrap --org acme --user alice --email alice@example.org
```

### `identity-manager validate` — validate the People config

The manager's `validate` operation is now a native verb (ADR-007 —
the convention end-state). It loads `config/identities/` and checks reference
