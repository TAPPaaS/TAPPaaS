...
// Mirrors people-manager/src/env.d.ts (the S-TS pilot pattern). Only the
// surface actually used here is declared; everything is typed (no implicit
// `any`) to satisfy strict mode.
...
declare module "fs" {
  ...
  // Used by the unit tests (writable temp config tree); harmless to the manager.
  export function mkdirSync(path: string, options?: { recursive?: boolean }): void;
  export function mkdtempSync(prefix: string): string;
}

// node:os — tmpdir() for the unit tests' writable config copy.
declare module "os" {
...
// ── node:child_process (the backup-controller FFI boundary) ────────────
