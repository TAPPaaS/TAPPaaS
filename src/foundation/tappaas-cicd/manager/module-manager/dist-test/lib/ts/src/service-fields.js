...
exports.SIDE_EFFECTS = exports.NORMALIZERS = exports.APPLY_MODES = exports.CHANGE_CLASS_NAMES = exports.CHANGE_CLASSES = void 0;
exports.worstClass = worstClass;
exports.isChangeClass = isChangeClass;
exports.parseServiceFieldManifest = parseServiceFieldManifest;
exports.effectiveApply = effectiveApply;
exports.lintServiceFieldManifest = lintServiceFieldManifest;
exports.needsActualState = needsActualState;
exports.manifestRelPath = manifestRelPath;
exports.ownedFieldsFor = ownedFieldsFor;
exports.CHANGE_CLASSES = {
    immutable: {
        applies: false,
        preGate: true,
        disruptive: false,
...
function worstClass(classes) {
    ...
    for (const c of classes) {
        const spec = exports.CHANGE_CLASSES[c];
        if (!spec)
            continue;
        if (best === null || spec.rank > exports.CHANGE_CLASSES[best].rank)
            best = c;
    }
    return best;
}
exports.CHANGE_CLASS_NAMES = Object.keys(exports.CHANGE_CLASSES);
function isChangeClass(s) {
    return Object.prototype.hasOwnProperty.call(exports.CHANGE_CLASSES, s);
}
// ── the apply modes ────────────────────────────────────────────────────
//
...
// already is.
exports.APPLY_MODES = ["set", "hook", "composite", "reconcile", "none"];
...
exports.SIDE_EFFECTS = ["reboot", "dns", "wait-ip", "ha-repoint"];
...
// rest.
function parseServiceFieldManifest(doc, findings) {
...
function effectiveApply(e) {
    if (e.apply)
        return e.apply;
    const spec = exports.CHANGE_CLASSES[e.class];
    if (spec && !spec.applies)
        return "none";
    return "set";
}
...
function lintServiceFieldManifest(m, opts, findings) {
    const err = (message) => void findings.push({ severity: "error", message });
    ...
    const entries = [
        ...Object.entries(m.fields).map(([n, e]) => [n, e, false]),
        ...Object.entries(m.composites).map(([n, e]) => [n, e, true]),
    ];
    for (const [name, e, isComposite] of entries) {
        ...
        if (typeof e.class !== "string" || !isChangeClass(e.class)) {
            err(`${what}: unknown change class '${String(e.class)}' — must be one of: ${exports.CHANGE_CLASS_NAMES.join(" ")}`);
            continue; // every later rule keys off a valid class
        }
        const spec = exports.CHANGE_CLASSES[e.class];
        if (e.apply !== undefined && !exports.APPLY_MODES.includes(e.apply)) {
            err(`${what}: unknown apply mode '${e.apply}' — must be one of: ${exports.APPLY_MODES.join(" ")}`);
            continue;
        }
        ...
        if (!spec.applies && apply !== "none") {
            err(`${what}: class '${e.class}' is never applied by the converge, but declares apply:'${apply}' — ` +
        ...
        if (isComposite) {
            ...
            if (!Array.isArray(c.inputs) || c.inputs.length === 0) {
                ...
                for (const input of c.inputs) {
                    ...
                    if (typeof fe.class === "string" && isChangeClass(fe.class))
                        inputClasses.push(fe.class);
                }
                ...
                if (ceiling !== null && ceiling !== e.class) {
                    err(`${what}: class '${e.class}' does not match its inputs — the most-escalated input class is '${ceiling}'`);
                }
            }
        }
        else {
            ...
            if (!declared.has(name)) {
                err(`${what}: not declared by any field tier — a manifest may only classify fields the schema defines (#567: schemas/module-fields.json + <module>/fields.json + this file)`);
            }
            if (apply === "composite") {
                if (!f.composite) {
        ...
        if (e.normalize !== undefined && !exports.NORMALIZERS.includes(e.normalize)) {
            err(`${what}: unknown normalize '${e.normalize}' — must be one of: ${exports.NORMALIZERS.join(" ")}`);
        }
        for (const s of e.sideEffects ?? []) {
            if (!exports.SIDE_EFFECTS.includes(s)) {
                err(`${what}: unknown side effect '${s}' — must be one of: ${exports.SIDE_EFFECTS.join(" ")}`);
            }
        }
    ...
    if (missing.length > 0) {
        err(`fields.json does not classify ${missing.length} field(s) the composed schema says '${opts.coordinate}' owns: ` +
...
function needsActualState(m) {
    const applied = (e) => {
        const a = effectiveApply(e);
        return a === "set" || a === "hook" || a === "composite";
    };
    return (Object.values(m.fields).some(applied) || Object.values(m.composites).some(applied));
}
...
function manifestRelPath(service) {
...
// what "owned" means.
function ownedFieldsFor(coordinate, schema) {
    const out = [];
    for (const [name, spec] of Object.entries(schema)) {
        const usedBy = Array.isArray(spec?.usedBy) ? spec.usedBy : [];
        if (usedBy.includes(coordinate))
            out.push(name);
    }
    return out.sort();
}
