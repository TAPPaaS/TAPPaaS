...
exports.isNonModuleBasename = isNonModuleBasename;
exports.discoverModules = discoverModules;
exports.declaresBackup = declaresBackup;
...
const NON_MODULE_BASENAMES = new Set([
    "zones",
    "site",
    "module-fields",
    "cert-refids",
...
function isNonModuleBasename(name) {
 ...
 */
function discoverModules(configDir) {
    if (!(0, fs_1.existsSync)(configDir))
        return [];
    const out = [];
    for (const f of (0, fs_1.readdirSync)(configDir)) {
        if (!f.endsWith(".json"))
            continue;
        if (f.endsWith(".orig"))
            continue;
        const name = (0, path_1.basename)(f, ".json");
        if (isNonModuleBasename(name))
            continue;
        let raw;
        try {
            raw = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(configDir, f), "utf8"));
        }
        catch {
            continue;
        }
 ...
 */
function declaresBackup(raw, capability = "any") {
    const declared = [
