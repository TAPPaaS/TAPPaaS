...
const help_1 = require("../../../lib/ts/src/help");
const cli_1 = require("../../../lib/ts/src/cli");
Object.defineProperty(exports, "warn", { enumerable: true, get: function () { return cli_1.warn; } });
...
function usage() {
    (0, cli_1.info)((0, help_1.renderHelp)(HELP));
}
function parseOpts(args) {
    const o = {
        configDir: (0, config_1.defaultConfigDir)(),
        json: false,
        diff: false,
        apply: false,
        allowFork: false,
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        const next = () => {
            const v = args[i + 1];
            if (v === undefined)
                (0, cli_1.die)(`${a} requires an argument`);
            i++;
            return v;
        };
        if (a === "--config-dir") {
            ...
            if (a === "--variant")
                (0, cli_1.warn)(`--variant is deprecated; treating as --environment (ADR-007 P5)`);
            o.environment = next();
        }
        else if (a === "--vmid") {
            o.vmid = next();
...
function parseIntStrict(s, flag) {
    ...
    if (!Number.isInteger(n))
        (0, cli_1.die)(`${flag} requires an integer (got '${s}')`);
    return n;
}
...
function cmdListResolution(opts) {
    const mods = (0, config_1.listModules)(opts.configDir);
    const rows = mods.map((m) => (0, config_1.classifyModuleResolution)(opts.configDir, m.name));
    const unresolvable = rows.filter((r) => r.path === "unresolvable" || r.path === "broken-location");
    if (opts.json) {
        (0, cli_1.info)(JSON.stringify({ modules: rows, unresolvable: unresolvable.map((r) => r.module) }, null, 2));
        return unresolvable.length > 0 ? 1 : 0;
    }
    if (rows.length === 0) {
        (0, cli_1.info)(`(no deployed modules in ${opts.configDir})`);
        return 0;
    }
    ...
    (0, cli_1.info)(line(headers));
    (0, cli_1.info)(line(widths.map((w) => "-".repeat(w))));
    for (const row of body)
        (0, cli_1.info)(line(row));
    const counts = new Map();
    for (const r of rows)
        counts.set(r.path, (counts.get(r.path) ?? 0) + 1);
    console.log("");
    (0, cli_1.info)([...counts.entries()]
    ...
    if (broken.length > 0) {
        console.log("");
        (0, cli_1.warn)(`${broken.length} module(s) record a .location that no longer exists: ` +
            broken.map((r) => `${r.module} (${r.dir})`).join(", "));
        (0, cli_1.warn)("The checkout moved or was removed — restore it, or re-install the module.");
    }
    if (none.length > 0) {
        console.log("");
        (0, cli_1.warn)(`${none.length} module(s) cannot be located by any tracking path: ` +
            none.map((r) => r.module).join(", "));
        (0, cli_1.warn)("No .location recorded and no repository catalog entry. Any operation needing " +
...
function cmdList(opts, client) {
    if (opts.resolution)
        return cmdListResolution(opts);
    if (opts.diff)
        return cmdListDiff(opts, client);
    const mods = (0, config_1.listModules)(opts.configDir);
    ...
    const configVmids = new Set(mods.map((m) => m.vmid).filter((v) => v != null));
    ...
    const notRunning = resolved.filter((r) => r.m.vmid != null && live && !byVmid.has(r.m.vmid));
    if (opts.json) {
        ...
        (0, cli_1.info)(JSON.stringify(live ? out : summary, null, 2));
        return 0;
    }
    if (mods.length === 0 && templateGuests.length === 0) {
        (0, cli_1.info)(`(no deployed modules in ${opts.configDir})`);
        if (!live)
            (0, cli_1.info)(`${cli_1.YW}[Warning]${cli_1.CL} live cluster query unavailable — showing config only`);
        return 0;
    }
    ...
    const fmt = (cells) => cells.map((c, i) => c.padEnd(w[i])).join("  ").trimEnd();
    (0, cli_1.info)(`${cli_1.GN}${fmt(headers)}${cli_1.CL}`);
    for (const r of rows)
        (0, cli_1.info)(fmt(r));
    if (!live) {
        (0, cli_1.info)(`${cli_1.YW}[Warning]${cli_1.CL} live cluster query unavailable — showing config only`);
    ...
    for (const r of notRunning) {
        (0, cli_1.info)(`${cli_1.YW}[Note]${cli_1.CL} ${r.m.name} (vmid ${r.m.vmid}) is configured but not running`);
    }
    // Genuine orphans (non-template running VMs in no module config).
    if (trueOrphans.length > 0) {
        (0, cli_1.info)("");
        (0, cli_1.info)(`${cli_1.YW}Unexpected VMs (running, not in any module config):${cli_1.CL}`);
        for (const g of trueOrphans)
            (0, cli_1.info)(`  ${g.vmid}  ${g.name}  ${g.node}  (${g.status})`);
    }
    return 0;
}
...
function cmdListDiff(opts, client) {
    const mods = (0, config_1.listModules)(opts.configDir);
    if (mods.length === 0) {
        (0, cli_1.info)(`(no deployed modules in ${opts.configDir})`);
        return 0;
    }
    (0, cli_1.info)(`${cli_1.GN}Per-module three-way drift (Released[git] / Desired[~/config] / Actual[running VM]):${cli_1.CL}`);
    ...
    if (!checkServices) {
        (0, cli_1.info)(`${cli_1.YW}(dependency-service state NOT checked — pass --services to include it, at one check per dependency per module)${cli_1.CL}`);
    }
    let worst = 0;
    for (const m of mods) {
        (0, cli_1.info)("");
        (0, cli_1.info)(`${cli_1.GN}── ${m.name} ──${cli_1.CL}`);
...
function cmdShow(opts) {
    ...
    if (!name)
        (0, cli_1.die)("show: expected <module>");
    const m = (0, config_1.loadModule)(opts.configDir, name);
    if (!m)
        (0, cli_1.die)(`module '${name}' not found in ${opts.configDir}`);
    ...
    (0, cli_1.info)(JSON.stringify(m.raw, null, 2));
    return 0;
}
...
// 'host' on the cluster.
function cmdResolveVerb(opts) {
    const name = opts.rest[0];
    if (!name)
        (0, cli_1.die)("resolve: expected <module>");
    return (0, resolve_1.cmdResolve)(name, { configDir: opts.configDir, json: opts.json });
}
...
function cmdDriftVerb(opts) {
    const name = opts.rest[0];
    if (!name)
        (0, cli_1.die)("drift: expected <module>");
    return (0, converge_1.cmdDrift)(name, { configDir: opts.configDir, json: opts.json, service: opts.service });
}
...
function readDeclaredRepositories(configDir) {
    try {
        const raw = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(configDir, "site.json"), "utf8"));
        return (raw.repositories ?? [])
            .filter((r) => typeof r.path === "string" && r.path !== "")
            .map((r) => ({ name: r.name ?? "", path: r.path, branch: r.branch ?? "" }));
    }
    catch {
        return [];
    }
...
function cmdValidate(opts) {
    ...
    if (name) {
        ...
        mods = (0, config_1.listModules)(opts.configDir);
    }
    ...
    const report = (0, validate_1.validateModules)(mods, {
        allowFork: opts.allowFork,
        fs: (0, services_1.realServiceFs)(opts.configDir),
        ...
        schema: (0, desired_1.loadModuleFields)(opts.configDir),
    });
    if (opts.json) {
        (0, cli_1.info)(JSON.stringify(report, null, 2));
        return report.errors > 0 ? 1 : 0;
    }
    for (const f of report.findings) {
        if (f.severity === "error") {
            console.error(`${cli_1.RD}[Error]${cli_1.CL} ${f.module}: ${f.message}`);
        }
        else {
            (0, cli_1.warn)(`${f.module}: ${f.message}`);
        }
    }
    if (report.errors > 0) {
        (0, cli_1.die)(`validate FAILED: ${report.errors} error(s), ${report.warnings} warning(s) across ${mods.length} module(s)`);
    }
    (0, cli_1.info)(`${cli_1.GN}validate ok${cli_1.CL} (${mods.length} module(s), ${report.warnings} warning(s))`);
    return 0;
}
// ── LIFECYCLE verbs (delegate to bash via the ModuleClient) ────────────
function cmdAdd(opts, client) {
    const module = opts.rest[0];
    if (!module)
        (0, cli_1.die)("add: expected <module>");
    const a = {
...
function addFieldOverrides(opts) {
    const passthrough = [...opts.passthrough];
    if (opts.vmid !== undefined)
        passthrough.push("--vmid", opts.vmid);
    if (opts.zone0 !== undefined)
        passthrough.push("--zone0", opts.zone0);
    return passthrough;
}
...
function cmdModify(opts, client) {
    const module = opts.rest[0];
    if (!module)
        (0, cli_1.die)("modify: expected <module>");
...
function applySets(module, opts) {
    const requests = [];
    for (const raw of opts.sets) {
        const parsed = (0, converge_1.parseSetArg)(raw);
        if (!parsed) {
            console.error(`${cli_1.RD}[Error]${cli_1.CL} --set '${raw}' is not field=value`);
            return 1;
        }
        requests.push(parsed);
    }
    const gate = (0, converge_1.preGateSet)(opts.configDir, module, requests, (0, desired_1.loadModuleFields)(opts.configDir));
    for (const w of gate.warnings)
        (0, cli_1.warn)(w);
    if (!gate.ok) {
        for (const r of gate.rejections) {
            console.error(`${cli_1.RD}[Error]${cli_1.CL} --set ${r.field}: ${r.reason}`);
        }
        console.error(`${cli_1.RD}[Error]${cli_1.CL} nothing was written — a modify either applies every --set or none of them`);
        return 1;
    }
    for (const p of gate.plan) {
        const where = p.coordinate ? `${p.coordinate}, ${p.class}` : "config-only";
        (0, cli_1.info)(`  ${p.field}=${p.value}  (${where})`);
    }
    // The write itself is bash: it must be Pattern-A aware and must run as the
    // operator, never root (#525). See set-module-field.sh.
    const args = [module];
    for (const p of gate.plan)
        args.push("--set", `${p.field}=${p.value}`);
    const rc = (0, exec_1.stream)(process.env.TAPPAAS_SET_FIELD_BIN ?? "set-module-field.sh", args);
    if (rc !== 0) {
        console.error(`${cli_1.RD}[Error]${cli_1.CL} writing the --set fields failed — the config may be partially updated; ` +
            `check 'module-manager module show ${module}'`);
        return rc;
    }
    return 0;
}
function cmdDelete(opts, client) {
    const module = opts.rest[0];
    if (!module)
        (0, cli_1.die)("delete: expected <module>");
    if (opts.archive && opts.remove)
        (0, cli_1.die)("delete: --archive and --remove are mutually exclusive");
    const d = {
        environment: opts.environment,
...
function cmdTest(opts, client) {
    const module = opts.rest[0];
    if (!module)
        (0, cli_1.die)("test: expected <module>");
    const t = { deep: opts.deep, vmid: opts.vmid, zone0: opts.zone0 };
...
function cmdReconcile(opts, client) {
    const module = opts.rest[0];
    if (!module)
        (0, cli_1.die)("reconcile: expected <module>");
    if (!opts.apply) {
...
// which was intended.
function cmdMigrate(opts, client) {
    const module = opts.rest[0];
    if (!module)
        (0, cli_1.die)("migrate: expected <module>");
    if (opts.rest.length > 1) {
        (0, cli_1.die)(`migrate takes no node argument (got '${opts.rest[1]}'). It moves the guest ` +
...
function cmdSnapshot(opts, client) {
    const module = opts.rest[0];
    if (!module)
        (0, cli_1.die)("snapshot-vm: expected <module>");
    let action;
    if (opts.snapList) {
        action = { kind: "list" };
    }
...
// reachable.
function dispatch(verb, opts, client) {
    switch (verb) {
        case "list":
        ...
        default:
            usage();
            (0, cli_1.die)(`Unknown verb: ${verb}`);
    }
}
function run(argv, client) {
    ...
    if (rest.some((a) => a === "-h" || a === "--help")) {
        (0, cli_1.info)((0, help_1.renderVerbHelp)(HELP, verb));
        return 0;
    }
    ...
    return (0, cli_1.guarded)(() => {
        (0, cli_1.preflightGuard)(); // #533: refuse root; self-heal config/repo ownership
...
if (require.main === module) {
    process.exit(run(process.argv.slice(2), new client_1.CliModuleClient()));
}
