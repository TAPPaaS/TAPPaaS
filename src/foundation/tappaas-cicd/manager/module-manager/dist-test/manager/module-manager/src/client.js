...
exports.CliModuleClient = void 0;
...
function run(bin, args) {
    try {
        const rc = (0, exec_1.stream)(bin, args);
        return rc === -1 ? 1 : rc;
    }
    catch (e) {
...
class CliModuleClient {
    add(module, opts) {
        const args = [module];
        if (opts.environment)
            args.push("--environment", opts.environment);
        if (opts.allowFork)
            args.push("--allow-fork");
        if (opts.force)
            args.push("--force");
        if (opts.reinstall)
            args.push("--reinstall");
    ...
    reconcile(module, opts) {
        return (0, reconcile_1.reconcileModule)(module, opts);
    }
    ...
    inspect(module, opts = {}) {
        return (0, inspect_1.inspectModule)(module, opts);
    }
    test(module, opts) {
        const args = [];
        if (opts.deep)
    ...
    clusterResources() {
        const candidates = (0, cluster_1.defaultNodeCandidates)((0, config_1.siteNodeHostnames)((0, config_1.defaultConfigDir)()));
        const nodes = (0, cluster_1.reachableNodes)(candidates);
        if (nodes.length === 0)
            return [];
        const guests = (0, cluster_1.queryClusterGuests)(nodes[0]);
        return guests ?? [];
    }
}
exports.CliModuleClient = CliModuleClient;
