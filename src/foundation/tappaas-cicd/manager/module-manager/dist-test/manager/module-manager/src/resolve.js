...
exports.resolveModuleFromConfig = resolveModuleFromConfig;
exports.renderResolved = renderResolved;
...
function resolveModuleFromConfig(module, configDir) {
    const cfg = readNormalized((0, path_1.join)(configDir, `${module}.json`));
    if (cfg === null)
        return null;
    const orig = readNormalized((0, path_1.join)(configDir, `${module}.json.orig`));
    return (0, desired_1.resolveModule)(module, cfg, orig, (0, desired_1.loadModuleFields)(configDir));
}
...
function renderResolved(r) {
    ...
    const out = [
        `${shlog_1.GN}Desired state for ${r.module}${shlog_1.CL} (deployed config + module-fields.json defaults)`,
        "",
        `${shlog_1.GN}${line(headers)}${shlog_1.CL}`,
    ...
    if (!r.origAvailable) {
        out.push(`${shlog_1.YW}(no <module>.json.orig pre-image — whether a field is deliberately off its release is unknown)${shlog_1.CL}`);
    }
    return out;
}
// The verb. Returns the exit code; errors are returned, never thrown, matching
// the other config-layer verbs.
function cmdResolve(module, opts) {
    const r = resolveModuleFromConfig(module, opts.configDir);
    if (r === null) {
        (0, shlog_1.error)(`Module config not found: ${(0, path_1.join)(opts.configDir, `${module}.json`)} — is '${module}' installed?`);
        return 1;
    }
    if (opts.json) {
        (0, shlog_1.emitJson)(r);
        return 0;
    }
    for (const l of renderResolved(r))
        (0, shlog_1.info)(l);
    return 0;
}
