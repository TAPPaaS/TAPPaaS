...
exports.VALID_SOURCES = exports.VALID_TIERS = void 0;
exports.validateModule = validateModule;
exports.validateSourceLocation = validateSourceLocation;
exports.validateConfigBlock = validateConfigBlock;
exports.validateDependsOn = validateDependsOn;
exports.validateIntegratesWith = validateIntegratesWith;
exports.validateFieldManifests = validateFieldManifests;
exports.validateModules = validateModules;
const path_1 = require("path");
const service_fields_1 = require("../../../lib/ts/src/service-fields");
const services_1 = require("./services");
const types_1 = require("./types");
exports.VALID_TIERS = ["foundation", "app"];
exports.VALID_SOURCES = ["official", "community", "private", "local"];
// Lint one module config; append findings to `out`.
function validateModule(m, opts, out) {
    const err = (message) => void out.push({ module: m.name, severity: "error", message });
    const warn = (message) => void out.push({ module: m.name, severity: "warning", message });
    ...
    else if (!exports.VALID_TIERS.includes(tier)) {
        err(`invalid tier '${tier}' (must be one of: ${exports.VALID_TIERS.join(" ")})`);
    }
    // source — default 'official' when absent (always valid).
    const source = m.source ?? "official";
    if (!exports.VALID_SOURCES.includes(source)) {
        err(`invalid source '${source}' (must be one of: ${exports.VALID_SOURCES.join(" ")})`);
    }
    ...
    if (tier === "foundation" && source !== "official") {
        if (opts.allowFork) {
            ...
            err(`tier:foundation requires source:official (got '${source}'). Pass --allow-fork to permit a foundation fork.`);
        }
    }
    ...
    if (status && !types_1.MODULE_STATUS_VALUES.includes(status)) {
        warn(`unknown status '${status}' — expected one of: ${types_1.MODULE_STATUS_VALUES.join(" ")}`);
    }
    ...
    if (opts.fs)
        validateDependsOn(m, opts.fs, out);
    if (opts.fs)
        validateIntegratesWith(m, opts.fs, out);
    // source-ref derivability — see validateSourceLocation.
    if (opts.repos)
        validateSourceLocation(m, opts.repos, out);
    // ADR-020 service field manifests — see validateFieldManifests.
    if (opts.fs && opts.schema)
        validateFieldManifests(m, opts.fs, opts.schema, out);
...
function validateSourceLocation(m, repos, out) {
    const raw = m.raw;
    ...
    void out.push({
        module: m.name,
        severity: "warning",
        message: `source at '${loc}' is outside every repository declared in site.json — ` +
...
function validateConfigBlock(m, out) {
...
function validateDependsOn(m, fs, out) {
    ...
    for (const dep of deps) {
        ...
        const svcScript = (0, path_1.join)(dir, "services", service, "update-service.sh");
...
// dependsOn raises.
function validateIntegratesWith(m, fs, out) {
    const integ = Array.isArray(m.integratesWith) ? m.integratesWith : [];
    if (integ.length === 0)
        return;
    const hard = new Set((Array.isArray(m.dependsOn) ? m.dependsOn : []).filter((d) => typeof d === "string"));
    const environment = typeof m.environment === "string" ? m.environment : "";
...
function validateFieldManifests(m, fs, schema, out) {
    ...
    for (const dep of deps) {
        ...
        if (seen.has(coordinate))
            continue;
        seen.add(coordinate);
        const owned = (0, service_fields_1.ownedFieldsFor)(coordinate, schema);
        ...
        if (text === null) {
            if (owned.length > 0) {
                out.push({
                    module: m.name,
                    severity: "error",
                    message: `${coordinate} owns ${owned.length} declared field(s) (${owned.join(", ")}) but ships no ` +
        ...
        try {
            ...
            out.push({
                module: m.name,
                severity: "error",
                message: `${providerModule} '${path}' is not valid JSON: ${e.message}`,
            });
            continue;
        }
        const manifest = (0, service_fields_1.parseServiceFieldManifest)(doc, findings);
        if (manifest) {
            (0, service_fields_1.lintServiceFieldManifest)(manifest, { coordinate, ownedFields: owned, declaredFields }, findings);
        }
...
function validateModules(modules, opts) {
    const findings = [];
    for (const m of modules)
        validateModule(m, opts, findings);
