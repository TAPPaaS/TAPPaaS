...
const converge_1 = require("../../src/converge");
...
const root = (0, fs_1.mkdtempSync)((0, path_1.join)((0, os_1.tmpdir)(), "mm-modify-"));
const configDir = (0, path_1.join)(root, "config");
const clusterDir = (0, path_1.join)(root, "src", "cluster");
(0, fs_1.mkdirSync)(configDir, { recursive: true });
(0, fs_1.mkdirSync)((0, path_1.join)(clusterDir, "services", "vm"), { recursive: true });
(0, fs_1.writeFileSync)((0, path_1.join)(configDir, "cluster.json"), JSON.stringify({ vmname: "cluster", location: clusterDir, kind: "module" }));
(0, fs_1.writeFileSync)((0, path_1.join)(configDir, "demo.json"), JSON.stringify({
    vmname: "demo",
    vmid: 300,
    kind: "module",
    dependsOn: ["cluster:vm"],
}));
(0, fs_1.writeFileSync)((0, path_1.join)(clusterDir, "services", "vm", "fields.json"), JSON.stringify({
    service: "cluster:vm",
    fields: {
...
(0, fs_1.writeFileSync)((0, path_1.join)(configDir, "module-fields.json"), JSON.stringify({ fields: SCHEMA }));
const gate = (...pairs) => (0, converge_1.preGateSet)(configDir, "demo", pairs.map((p) => (0, converge_1.parseSetArg)(p)), SCHEMA);
const rejected = (r, needle) => !r.ok && r.rejections.some((x) => x.reason.includes(needle));
try {
    // ── the scenarios the gate LETS THROUGH ──────────────────────────────
    {
        ...
        check(gate("diskSize=16G").ok, "M3 diskSize=16G (SHRINK) passes the gate — only the live size can tell, so the converge refuses it");
        ...
        check(m8.ok && m8.plan[0].value === "a new description", "a value containing spaces survives parsing intact");
    }
    ...
    // ── reject the WHOLE command (Resolved Question 5) ───────────────────
    {
        ...
        check(!mixed.ok && mixed.rejections.length === 1 && mixed.rejections[0].field === "vmid", "…naming only the field that is actually at fault");
    ...
    // ── parsing ──────────────────────────────────────────────────────────
    {
        check((0, converge_1.parseSetArg)("cores=8")?.value === "8", "field=value parses");
        check((0, converge_1.parseSetArg)("net0=virtio=02:AA,bridge=lan")?.value === "virtio=02:AA,bridge=lan", "only the FIRST '=' splits — a value may contain more");
        check((0, converge_1.parseSetArg)("cores") === null, "a bare field with no '=' is not a set");
        check((0, converge_1.parseSetArg)("=8") === null, "an empty field name is not a set");
        check((0, converge_1.parseSetArg)("description=")?.value === "", "an empty VALUE is legal — clearing a field is a change");
    }
    ...
    // every test above.
    {
        const setter = (0, path_1.join)(root, "fake-set-field.sh");
        const calls = (0, path_1.join)(root, "setter-calls");
        (0, fs_1.writeFileSync)(setter, `#!/usr/bin/env bash\necho "$*" >> "${calls}"\nexit 0\n`);
        (0, fs_1.chmodSync)(setter, 0o755);
        process.env.TAPPAAS_SET_FIELD_BIN = setter;
        const wrote = () => ((0, fs_2.existsSync)(calls) ? (0, fs_1.readFileSync)(calls, "utf8") : "");
        // Accepted: the writer runs, THEN the converge.
        {
            const client = new fake_client_1.FakeModuleClient();
            const rc = (0, main_1.run)(["modify", "demo", "--set", "cores=8", "--config-dir", configDir], client);
            check(rc === 0, "an accepted --set exits 0");
            check(wrote().includes("demo --set cores=8"), "…the value is handed to the writer");
            check(client.log.some((i) => i.verb === "modify" && i.module === "demo"), "…and the ordinary modify algorithm runs afterwards — one verb, one algorithm");
        }
        // Rejected: nothing is written and nothing converges.
        {
            (0, fs_1.rmSync)(calls, { force: true });
            const client = new fake_client_1.FakeModuleClient();
            const rc = (0, main_1.run)(["modify", "demo", "--set", "vmid=250", "--config-dir", configDir], client);
        ...
        // A mixed set stops the whole command, including the acceptable half.
        {
            (0, fs_1.rmSync)(calls, { force: true });
            const client = new fake_client_1.FakeModuleClient();
            (0, main_1.run)(["modify", "demo", "--set", "cores=8", "--set", "vmid=250", "--config-dir", configDir], client);
        ...
        {
            const client = new fake_client_1.FakeModuleClient();
            (0, main_1.run)(["modify", "demo", "--force", "--config-dir", configDir], client);
            const inv = client.log.find((i) => i.verb === "modify");
            check(!!inv && inv.opts.force === true, "modify --force reaches the converge as the disruption authorization");
        }
        delete process.env.TAPPAAS_SET_FIELD_BIN;
    }
    // A config dir with no schema at all: refuse, and say that is the reason.
    {
        const bare = (0, fs_1.mkdtempSync)((0, path_1.join)((0, os_1.tmpdir)(), "mm-noschema-"));
        try {
            const r = (0, converge_1.preGateSet)(bare, "demo", [(0, converge_1.parseSetArg)("cores=8")], {});
            check(!r.ok, "with no module-fields.json nothing is written");
            check(!r.ok && r.rejections[0].reason.includes("not readable"), "…and the reason is the missing schema, not a fabricated spelling complaint");
        }
        finally {
            (0, fs_1.rmSync)(bare, { recursive: true, force: true });
        }
    }
    ...
    {
        (0, fs_1.rmSync)((0, path_1.join)(clusterDir, "services", "vm", "fields.json"));
        const r = gate("cores=8");
        check(r.ok, "a provider with no manifest yet does not block a --set");
        check(r.warnings.some((w) => w.includes("no field manifest yet")), "…but the operator is told the change class is unknown");
    }
}
finally {
...
{
    const client = new fake_client_1.FakeModuleClient();
    const rc = (0, main_1.run)(["migrate", "demo"], client);
    check(rc === 0, "migrate <module> is accepted");
    check(client.log.some((i) => i.verb === "migrate" && i.module === "demo"), "…and reaches the client as a migrate");
    ...
    console.error = (...a) => { died += a.join(" ") + "\n"; };
    try {
        (0, main_1.run)(["migrate", "demo", "tappaas3"], c2);
    }
    catch (e) {
    ...
    check(!c2.log.some((i) => i.verb === "migrate"), "migrate <module> <node> is REFUSED — it never invents a destination");
    check(/--set node=tappaas3/.test(died), "…and the refusal names 'modify --set node=…', the verb that changes intent");
    const c3 = new fake_client_1.FakeModuleClient();
    (0, main_1.run)(["migrate", "demo", "--force"], c3);
    check(c3.log.some((i) => i.verb === "migrate" && i.opts.force === true), "--force is forwarded — an OFFLINE move is authorized explicitly, never implied");
}
console.log("");
