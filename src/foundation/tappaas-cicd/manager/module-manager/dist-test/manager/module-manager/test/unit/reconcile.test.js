...
const reconcile_1 = require("../../src/reconcile");
...
const ROOT = (0, fs_1.mkdtempSync)((0, path_1.join)((0, os_1.tmpdir)(), "mm-reconcile-"));
const CONFIG = (0, path_1.join)(ROOT, "config");
(0, fs_1.mkdirSync)(CONFIG, { recursive: true });
// Every stub appends "<label>|<argv>|<cwd>" to this log.
const LOG = (0, path_1.join)(ROOT, "calls.log");
function stub(path, label, exitCode = 0) {
    (0, fs_1.mkdirSync)((0, path_1.join)(path, ".."), { recursive: true });
    (0, fs_1.writeFileSync)(path, `#!/usr/bin/env bash\nprintf '%s|%s|%s\\n' "${label}" "$*" "$PWD" >> "${LOG}"\nexit ${exitCode}\n`);
    (0, fs_1.chmodSync)(path, 0o755);
}
function calls() {
    if (!(0, fs_1.existsSync)(LOG))
        return [];
    return (0, fs_1.readFileSync)(LOG, "utf8").trim().split("\n").filter(Boolean);
}
function resetLog() {
    if ((0, fs_1.existsSync)(LOG))
        (0, fs_1.rmSync)(LOG);
...
function makeProvider(name, services) {
    const dir = (0, path_1.join)(ROOT, "providers", name);
    (0, fs_1.mkdirSync)(dir, { recursive: true });
    (0, fs_1.writeFileSync)((0, path_1.join)(CONFIG, `${name}.json`), JSON.stringify({ location: dir }, null, 2));
    for (const s of services) {
        const kind = s.kind ?? SERVICE_SCRIPT;
        stub((0, path_1.join)(dir, "services", s.svc, kind), `${name}:${s.svc}:${kind}`, s.exit ?? 0);
    }
    return dir;
}
// A consuming module: <root>/modules/<name> with an update.sh, plus its config.
function makeModule(name, dependsOn, opts = {}) {
    const dir = (0, path_1.join)(ROOT, "modules", name);
    (0, fs_1.mkdirSync)(dir, { recursive: true });
    stub((0, path_1.join)(dir, "update.sh"), `${name}:update.sh`, opts.updateExit ?? 0);
    (0, fs_1.writeFileSync)((0, path_1.join)(CONFIG, `${name}.json`), JSON.stringify({ location: dir, dependsOn, vmid: 900, vmname: name }, null, 2));
    return dir;
}
const OPTS = { silent: false, debug: false };
...
// ── 1. happy path: dependency services then the module's own update.sh ──
{
    ...
    const rc = (0, reconcile_1.reconcileModule)("app-a", { ...OPTS });
    const c = calls();
    check(rc === 0, "happy path exits 0");
    check(c.some((l) => l.startsWith(`prov:thing:${SERVICE_SCRIPT}|app-a|`)), `Step 2 calls the provider's ${SERVICE_SCRIPT} with the consuming module name`);
    check(c.some((l) => l.startsWith("app-a:update.sh|app-a|")), "Step 3 runs the module's own update.sh");
    ...
    check(depCall.endsWith(`|${moduleDir}`), "Step 2 runs dependency services from the module directory (#495)");
...
// could restore what the rebuild dropped.
{
    ...
    const rc = (0, reconcile_1.reconcileModule)("app-b", { ...OPTS });
    const c = calls();
    check(rc === 1, "a failed dependency service still makes reconcile exit 1");
    check(c.some((l) => l.startsWith("app-b:update.sh|app-b|")), "Step 3 STILL runs after a Step 2 failure (#495 — never leave the module less converged)");
}
// ── 3. every dependency is attempted; one failure does not stop the rest ─
{
    ...
    const rc = (0, reconcile_1.reconcileModule)("app-c", { ...OPTS });
    const c = calls();
    check(rc === 1, "one failing dependency of two exits 1");
    check(c.some((l) => l.startsWith(`p2:b:${SERVICE_SCRIPT}|`)), "a later dependency still runs after an earlier one failed");
...
// (several dependsOn entries name providers with no services/ directory at all).
{
    ...
    const rc = (0, reconcile_1.reconcileModule)("app-d", { ...OPTS });
    check(rc === 0, `a provider with no ${SERVICE_SCRIPT} is skipped, not a failure`);
    check(calls().some((l) => l.startsWith("app-d:update.sh|")), "Step 3 still runs when a dependency was skipped");
}
// ── 5. module's own update.sh failing is fatal ──────────────────────────
{
    ...
    const rc = (0, reconcile_1.reconcileModule)("app-e", { ...OPTS });
    check(rc === 1, "a failing module update.sh exits 1");
}
// ── 6. an uninstalled module is refused before anything runs ────────────
{
    resetLog();
    const rc = (0, reconcile_1.reconcileModule)("not-installed-at-all", { ...OPTS });
...
// ── 7. no dependsOn: Step 3 alone ───────────────────────────────────────
{
    resetLog();
    makeModule("app-f", []);
    const rc = (0, reconcile_1.reconcileModule)("app-f", { ...OPTS });
