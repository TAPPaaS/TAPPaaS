...
// Mirrors people-manager / network-manager src/env.d.ts. Only the surface
...
// ── node:child_process (the bash-script FFI boundary) ─────────────────
declare module "child_process" {
  ...
  export interface SpawnSyncOptions {
    ...
    // the long-running install/update/delete/test scripts so their step-by-step
    // output is visible, exactly as the bash orchestrators do).
    stdio?: "inherit" | "pipe";
  }
