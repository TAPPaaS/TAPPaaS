...
`site-manager`. Operator decision (2026-09-19): nodes are **`pvehost`**, not `debianhost`, so
...
`pvehost`, Debian without → `debianhost` (`adopt_module_for_os`).
...
**Address.** The instance records `address: <node>.mgmt.internal`; `lib/pvehost-lib.sh` reaches
