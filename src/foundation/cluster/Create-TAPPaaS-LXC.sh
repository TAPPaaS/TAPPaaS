...
# die: report and stop, with the ERR trap off so the message is not buried under
# the generic handler (the VM path has the same helper).
die()   { trap - ERR; errln "$*"; exit 1; }
...
# #692: a container is always created on the local node, so that is what the
# pool is checked against. Same rule as Create-TAPPaaS-VM.sh.
STORAGE="$(resolve_storage "$(hostname -s)" 'tanka1')"
...
if [[ "$(get_meta '.gpu' '')" != "" ]]; then
  ...
  # /dev/kfd (and the render node) get their device major assigned dynamically at
  # host boot, so the value committed in <module>.meta.json is unreliable across
  # reboots and across hosts. This script runs ON the target node (cluster:lxc
  # install-service ssh), so resolve the majors LIVE from the actual device nodes;
  # fall back to the committed meta only when the device is absent. Without this a
  # stale committed major writes a cgroup allow the kernel denies → the bind-mounted
  # /dev/kfd is visible but unusable → "No HIP GPUs are available".
  if [[ -c /dev/kfd ]]; then
  ...
  if [[ -c "/dev/dri/${RENDER_NODE}" ]]; then
    RENDER_MAJOR="$(printf '%d' "0x$(stat -c '%t' "/dev/dri/${RENDER_NODE}")")"
    RENDER_MINOR="$(printf '%d' "0x$(stat -c '%T' "/dev/dri/${RENDER_NODE}")")"
  else
    RENDER_MAJOR="$(get_meta '.gpu.render_major')"
    RENDER_MINOR="$(get_meta '.gpu.render_minor')"
  fi
  info "Configuring GPU passthrough (kfd ${KFD_MAJOR}:${KFD_MINOR}, ${RENDER_NODE} ${RENDER_MAJOR}:${RENDER_MINOR}) [live-resolved]..."
  {
    echo ""
