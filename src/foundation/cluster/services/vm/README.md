...
| Type | `boolean` |
| Default | `false` |
| Allowed values | `true` — Fully unattended build — no console interaction required (e.g. tappaas-winserver with autounattend.xml)<br>`false` — Requires manual installation — operator must run `module-manager module add` and complete the setup (e.g. tapp |
| Required by | *(none)* |
| Used by | `cluster:vm` |
| Change class | `recreate` |
| Apply mode | `none` |
