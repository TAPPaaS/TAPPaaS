...
- **Test 4: node sshd is key-only** — on every node reads `sshd -T` (what sshd will *enforce*, so an `sshd_config` overriding the TAPPaaS drop-in is caught) and asserts `permitrootlogin without-password`, `passwordauthentication no`, `kbdinteractiveauthentication no`. In the pre-update gate (`TAPPAAS_TEST_RUNTIME_ONLY=1`) a node that is not key-only yet is **skipped, not failed** — `update.sh` Step 5 is what converges it, and failing would block that update.
