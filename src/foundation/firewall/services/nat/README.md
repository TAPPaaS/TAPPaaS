...
`firewall:nat` and declares the port mappings. The reverse-proxy service
(`firewall:proxy`) remains the right choice for HTTP(S) — use `firewall:nat`
...
Add `firewall:nat` to `dependsOn` and declare the mappings under the
`firewall:nat` config block:
...
```jsonc
{
  ...
  "dependsOn": [
    ...
    "firewall:rules",
    "firewall:dns",
    "firewall:proxy",   // web UI at https://forgejo.<domain>
    "firewall:nat"      // raw SSH on the WAN IP
  ],
  "config": {
    "firewall:nat": {
      "natRules": [
