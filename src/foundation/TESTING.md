...
|--------|:-------:|------|-----------|-----|
| cluster | ✅ | script presence + `vm-net.sh` unit suite (incl. the LXC `hwaddr=` NIC form) + `cluster:vm --check` (now the ADR-020 converge: manager-computed drift → `converge-lib.sh`) + **`report-service.sh` actual-state contract for vm AND lxc** (valid JSON on stdout only, every declared liveKey present, all values strings, reported node = cluster node, each NIC split consistently with its whole value) | ✅ live vm/ha/lxc reconcile (#192/#193/#203), and the ADR-020 D8 **deferral**: an unauthorized subnet change exits 0, prints DEFERRED: and leaves the guest untouched, then applies under `--force` | live migration not exercised |
...
| logging | ✅ | **live** 8 health probes (Loki/Grafana/Alloy/syslog) | ❌ **none** | liveness-only; ingest assertion weak; no retention/dashboards/alerting |
...
| tappaas-cicd | ✅ | config-reader unit tests + bin presence/load smoke + **`test-converge-lib.sh`** (the ADR-020 drift-record runner, fully stubbed: one batched set, migrate last, side effects once in reboot→wait-ip→dns order, `manual` warns while `immutable`/`recreate` fails, hook exit 10 = deferred/rc 0 and 20 = refused/rc 1, `--check` applies nothing) | ✅ live VM create/reinstall/snapshot-rollback/variants; re-dispatches manager+controller suites | Test 11 component checks shallow; silent skips when optional bins absent |

## Managers (all have `test.sh` ✅; all run offline unit suites in fast tier)

| Manager | Fast (unit) | Deep/live tier | Gap |
|---------|-------------|----------------|-----|
| identity-manager | reconcile + entity CRUD (incl. ref guards) | ✅ **strong** — live Authentik OIDC/forward-auth via fixture VMs | — |
