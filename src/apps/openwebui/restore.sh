...
# Example: ./restore.sh --from-instance openwebui-old.srvWork.internal
...
Examples:
    ...
    ${SCRIPT_NAME} --from-instance openwebui-old.srvWork.internal

    # Restore from local backup files
    ${SCRIPT_NAME} --from-path /tmp/openwebui-backups

    # Restore specific date from another instance
    ${SCRIPT_NAME} --from-instance openwebui-old.srvWork.internal --date 2026-03-09
EOF
}
