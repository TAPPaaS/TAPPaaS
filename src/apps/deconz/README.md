...
Primary audience: home user with Zigbee devices; Home Assistant administrator; SysAP (free@home) users.

Standalone Zigbee gateway (ConBee II) with a diyHue Hue-bridge front-end. deCONZ runs the
Zigbee network on a **dedicated NixOS VM** instead of inside Home Assistant. Two consumers,
two faces:
