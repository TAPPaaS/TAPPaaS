...
# consumer's network:rules install-service compiles it. NB cross-zone SSDP
# discovery additionally requires the network:discovery relay (UDP 1900).
#
...
info "deconz:hue-bridge install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole via consumer network:rules; SSDP relay via network:discovery)."
