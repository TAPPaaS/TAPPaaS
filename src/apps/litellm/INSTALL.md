...
```bash
install-module.sh litellm --node tappaas1 --zone0 srv-dev --vmid 399
```
...
```bash
ssh tappaas@litellm.srv-work.internal "sudo cat /etc/secrets/litellm.env"
```
...
`http://litellm.srv-work.internal:4000/ui` — log in with the master key.
...
```bash
ssh tappaas@litellm.srv-work.internal "journalctl -u podman-litellm -n 50"
```
...
Check VM is reachable: `nc -zv -w 5 litellm.srv-work.internal 4000`

**Master key lost**
```bash
ssh tappaas@litellm.srv-work.internal
...
```bash
ssh tappaas@litellm.srv-work.internal "systemctl status postgresql"
ssh tappaas@litellm.srv-work.internal "sudo -u postgres psql -c '\l'"
```
