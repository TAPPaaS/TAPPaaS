...
| GitHub repo fate | **Weekly push mirror — live (2026-09-06, #586)** | Codeberg→GitHub push mirror (168h interval) on all three repos (`TAPPaaS`, `Documentation`, `Community`); keeps discoverability/SEO without drift. Code only — issues/PRs stay Codeberg-only. |
...
- [x] Set up GitHub ← Codeberg **push mirror** *(2026-09-06, #586)* — native Forgejo push mirror, 168h (weekly) interval, on all three repos (`TAPPaaS`, `Documentation`, `Community`). Gate met: `stable` = `v2.0` (`61e91f0`) since 2026-07-21. First sync verified: `main`/`stable`/tags landed on GitHub; image-build Actions untouched (their trigger tags already present on GitHub). Sync-now on each stable release; code only — issues/PRs stay on Codeberg.
