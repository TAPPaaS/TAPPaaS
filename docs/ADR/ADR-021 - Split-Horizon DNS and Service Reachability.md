...
`ZONES.md` (the caddy-reach rule, and why it is deliberately not an `access-to` entry), and the
`service` archetype's documented `access-to` seed, which loses `dmz` (D3b).
