...
- **`site-manager update --force` runs every module's update now** (v0.8, #633). It stands in for the maintenance window, never for a module's consent: a module with `rebootOk: false` keeps its disruptive changes deferred. Only `module modify <module> --force`, typed for one module, overrides `rebootOk: false`.
