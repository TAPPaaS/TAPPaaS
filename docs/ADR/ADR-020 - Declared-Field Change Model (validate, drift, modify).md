...
| | |
|---|---|
| **Status** | **Accepted** (2026-09-15; v0.9 same day) — P0–P6, D8 and D9 built. Follow-ups: the D9 `adopt` verb; D10 enforcement (the pre-gate refusal, #654). |
| **Version** | 0.10 |
| **Date** | 2026-09-16 |
| **Author** | Lars Rossen |
...
| **Changelog** | v0.1 initial draft: the planner/actor split, the change-class taxonomy, the per-service field-change hook contract. v0.2 (operator decision): **one** `modify` verb — `--set` writes the field into config first, then the *unchanged* core modify algorithm runs (3-way merge → converge via the module's own `update.sh` + every `dependsOn` `update-service.sh`). ADR-020's job is to make that converge field-change-aware, not to add a second scoped apply path. v0.3 (operator resolved the 7 open questions): manifest = `services/<service>/fields.json`; change class keyed by the (field, service) **pair**; `modify` **never** writes back to the repo/release source (deployed-config-only, Released-drift is expected); a **static pre-gate** rejects an immutable/recreate `--set` before writing; a mixed `--set` set is **rejected whole**; the never-converge-provider edge is **out of scope for v1**; the resolver/converge core is **shared** in `lib/ts`. Status → Proposed. v0.4 (realization detailed — D7/D8): the **manager owns the single drift computation** (services only *report* actual and *apply* the drift the manager hands them); a per-service **`report-service.sh`** reports actual state (reused by test/health/update); provider-specific parse deleted from `inspect.ts`; disruption (reboot/offline-migrate) is authorized by a per-module **`rebootOk`** (default false) + an explicit `modify --force`, decoupled from `update-tappaas --force` ("run now"), and an unauthorized disruptive change is **deferred with a warning, exit 0**; every existing `update-service.sh` is migrated with **semantics preserved** (non-field-update logic stays put). v0.6 (operator decision, post-implementation): **D9** — install writes the *resolved* value of every owned, reportable field into the deployed config, so the schema `default` is desired state for every field and an undeclared field is compared like any other. This **retires v0.5's per-field opt-out**, which hid an undeclared field from the converge for the life of the module, in favour of always-visible drift resolved by change class — change, report, or adopt — plus a new config-only **`adopt`** verb for the `manual`/`recreate`/`immutable` classes that `--set` is pre-gated against. v0.7: **D10** — the 19 module-level fields (owned by no service) are declared in a module-scoped `schemas/fields.json` using the same vocabulary, closing the gap that let `--set` write tooling-owned fields like `kind` and `location`; the pre-gate refusal that would enforce it is **#567**, deferred until ADR-020 has landed estate-wide. v0.5: added a **phased implementation plan** (P0–P6, scoped to the measured inventory: 25 `update-service.sh` scripts, resolver embryo in `inspect.ts`, `cfg()` ladder in `cluster:vm`), a **test plan** tied to the existing suites + mutation discipline, and a **documentation-impact** analysis (which `.md` files carry it, per ADR-013). v0.8 (acceptance): D8 names the fleet-wide lever — `site-manager update --force` runs now and respects `rebootOk` (#633); `modify --force` no longer overrides a failing pre-update test (#635); the scheduled pass is the site's `automaticReboot`. D10's follow-up was mislabelled #567, which was the fields.json split. Status → Accepted. v0.9 (2026-09-15, operator decision): `site-manager update --force` also passes `--ignore-test-failure` to every module; `rebootOk` is still never overridden fleet-wide; an operator run without `--force` opens no disruption window. v0.10 (2026-09-16, operator decision on #453/#655): one meaning for `--force` at every level — proceed, never reboot, never overwrite a deployed config; disruption becomes `--allow-disruption` and `--ignore-test-failure` is retired into `--force`; D5 gains the `update` verb (#655) and states that `add --force` refuses on a deployed module (#453). |
...
| **update** `<m>` | schema + config + live | config + cluster | write | the **release update** (#655): the core algorithm with no field change — snapshot → 3-way merge (local modifications survive) → converge → test → `updateTime`. This is what the sweep calls, and the word an operator reaches for. |
| **modify** `<m> --set field=value …` | schema + config + live | config + cluster | write | the **field change** (D2), and nothing else: `--set` writes the field(s) first, then the *same* core algorithm runs: snapshot → 3-way merge → converge (module `update.sh` + every `dependsOn` `update-service.sh`, now change-class-aware) → test → `updateTime`. |
...
`update` and `modify` are one engine with one difference — whether a field is written before
the converge. Bare `modify` remains accepted as an alias of `update` for one release, with a
deprecation warning: two spellings of the release update is what #655 reports as confusing.

**`add` installs; it does not re-write a deployed config** (#453, v0.10). `add <m> --force`
re-ran the installer over a deployed module and wrote the catalog template over the site's own
config — a custom environment and ip were lost, and the version went backwards (2026-08-16).
`add --force` therefore **refuses** on an already-deployed module and names the two real paths:
`module update <m>` to take the release forward keeping local modifications, or
`add --reinstall` for the documented delete-then-recreate. Nothing but `--reinstall` may
overwrite a deployed config.
...
### D8. Disruption authorization — `--force` proceeds, `--allow-disruption` reboots

**One word, one meaning, at every level (v0.10).** `--force` means *proceed with this update
although something says no*: a pre-update test that failed fatally, or a module whose status is
`archived`/`external`. It never overwrites a deployed config (D5, #453) and it never reboots a
guest. `site-manager update --force` therefore forwards `--force` to every module update, and
that is all it does to them — the lever an operator reaches for at the fleet level means there
what it means for one module.

Earlier versions of this ADR split the word instead: v0.8 made `modify --force` the disruption
lever and moved the test override to `--ignore-test-failure` (#635), and v0.9 had the fleet flag
pass that second flag. Three levels each carrying a different `--force` is what #655 reports as
confusing, so v0.10 gives disruption its own name and retires `--ignore-test-failure` (one
release old, never on `stable`) back into `--force`.

**Disruption is authorized by `--allow-disruption`**, per module (`module update <m>
--allow-disruption`) or for a run (`site-manager update --allow-disruption`), and never by
`--force`. Forwarding a routine update's flag as disruption authority would let it silently
reboot production guests.
...
- **Disruption is authorized iff** `--allow-disruption` was passed for that module or that run, **OR** `rebootOk == true` **AND** the pass is scheduled with the site's `automaticReboot` on (`update-tappaas` exports `TAPPAAS_SCHEDULED_PASS=1`, the same signal that authorizes node reboots). A plain operator run opens no window, even with `automaticReboot` on (v0.9; ADR-017 v0.2 D8 lets the unit tell the two runs apart).
- **`--allow-disruption` at the fleet level still respects `rebootOk`** (#633): it opens the window for modules that declare `rebootOk: true`; a module with `rebootOk: false` keeps its disruptive changes deferred. Only `module update <module> --allow-disruption`, typed for one module, overrides `rebootOk: false` — that is an operator standing in front of one workload, not a fleet-wide instruction.
- **`--force` proceeds; it neither reboots nor overwrites** (v0.10). It carries a fatally failed pre-update test (exit 2) and an `archived`/`external` status. The pre-update gate still follows the suite's own grading (#635): exit 2 aborts unless `--force`, exit 1 proceeds with a `TEST-WARN:` line, and the post-update test fails the module only on assertions that passed before the update.
...
Three distinct levers that no longer collide: **`--force`** (proceed past a refusal — same
meaning for one module and for the fleet), **`--allow-disruption`** (authorize downtime now),
and **`module.rebootOk`** (standing per-module consent, honoured in the scheduled pass and by a
fleet `--allow-disruption`). Starting the sweep at all is `site-manager update`, not a flag.

### D9. Config records what install used — the schema default is desired state

**The problem.** `module-fields.json` gives each field one `default`, and two different consumers read it: *install*, asking "what do I build with if the module said nothing?", and *the converge*, asking "what do I enforce if the module said nothing?". For most fields those coincide, because TAPPaaS's own creators build with exactly the schema default. Where they diverge the converge is left enforcing a value nobody chose — a guest created at the `diskSize` default of `8G` and later grown to 80G would have the converge read `8G` as desired and refuse a shrink on every pass, forever. The v0.5 implementation carried a per-field opt-out that made the converge *skip* such a field entirely: safe, but the field was then invisible for the life of the module and the deployed config stayed permanently incomplete.

**The decision.** **Install writes the resolved value of every owned, reportable field into the deployed config**, whether that value came from the module's own declaration or from the schema default. After install, config and actual agree *by construction*. Therefore:

1. The schema `default` is **desired state for every field**, and the converge compares and applies it exactly as it would a declared value. No per-field opt-out, no sixth apply mode, no third axis (see Resolved Question 12).
2. Where install cannot rely on the creator having applied that default — a guest built elsewhere and adopted — install records what it actually **observed** instead.
3. Any later divergence is therefore a *real* post-install change, and is **always reported** — never skipped.

**Resolution is by change class.** Reporting a drift is not the same as resolving it; the class already says which of three things may happen, and this decision only makes the drift visible enough to act on:

| Class | On drift, the platform… | How an operator resolves it |
|---|---|---|
| `in-place`, `in-place-reboot`, `migrate` | **changes** — the converge applies config onto actual (disruptive classes still gated by D8) | nothing to do; or `--set` to a new value |
| `grow-only` | **changes** on a grow; refuses a shrink at apply time | `--set` the config up to the live size |
| `manual` | **reports**, every run, until it is fixed | `--set`/`adopt` to accept reality, **or** change reality (move the disk back) |
| `recreate`, `immutable` | **reports**, every run | `--set` is pre-gated (D4) and cannot be used — only `adopt`, or a rebuild |

**Worked example — `diskSize` (`grow-only`).** A module declares no size, so install creates the guest at the schema default and records `"diskSize": "8G"`. The disk is later grown to 80G outside TAPPaaS. Config says 8G, actual says 80G: a drift, reported on every converge, and not applied — a shrink is refused. The operator resolves it with the ordinary verb, `module-manager module modify <m> --set diskSize=80G`, which writes config and runs the converge, finding nothing left to apply. Under v0.5 this drift was invisible.

**Worked example — `storage` (`manual`).** Config records `tanka1`; the disk is later moved to `tankb1`. `manual` never acts, so this is reported on **every** run until it is fixed — either by moving the disk back, or by taking the new location as the declaration.

**A new verb: `module-manager module adopt <field>…`** — write the *reported actual* into the deployed config for the named fields, touching nothing on the cluster. It is not a synonym for `--set`:

- `--set` requires the operator to transcribe the value; `adopt` reads it from `report-service.sh`, so it cannot be mistyped.
- `--set` is **pre-gated** for `recreate` and `immutable` (D4), because config must never claim something the converge could only refuse. `adopt` moves config *toward* reality rather than away from it, so the pre-gate does not apply and it is the **only** way to reconcile a `recreate`/`immutable` drift short of a rebuild.
- Intent is explicit and auditable: "reality is now the declaration."

`adopt` on an `immutable` field is the one case that should still refuse without `--force`: a changed `vmid` or `image` does not mean the declaration was stale, it means the config is pointing at a different guest, and silently adopting would paper over an error. *(Operator's spelling was `adapt`; `adopt` is used here as the conventional term for taking existing state under management — the verb name is not settled.)*

**`not-reported` is a reporter gap, not a property of the field.** Six `cluster:vm` fields — `autoInstall`, `sshAccess`, `os`, `imageType`, `image`, `cloudInit` — are skipped as `not-reported` today. That is a statement about what `report-service.sh` currently looks at, and for half of them it is simply a gap worth closing. Two categories, and they should stop being conflated:

- **Observable, and now observed.** `cloudInit` is the presence of the guest's cloudinit drive (`<bus><n>: <storage>:vm-<id>-cloudinit`), read from the `qm config` text the reporter already holds — so it is reported for *every* guest, and `false` means the guest genuinely has none. `os` is `qm agent <id> get-osinfo`, whose distro id the reporter maps into this schema's vocabulary (Windows spells itself `mswindows`). Both are implemented. Their contracts deliberately differ: the agent only answers for a **running** guest that has it, so when it cannot, the `os` key is **omitted rather than emitted empty** — an empty string would read as "this guest has no OS" and drift against every module that declares one.
- **Observable in principle, but not by any proxy we have.** `sshAccess` means "TAPPaaS can manage this guest over SSH". The tempting proxy is the `sshkeys:` line in `qm config`, and it is **wrong on this estate's own firewall**: `module-fields.json` defines `sshAccess: true` as "NixOS VMs, **OPNsense as root**", and OPNsense is managed over SSH as root with no cloud-init keys — so the proxy would report `false` against a declared `true`, on a `recreate` field, which is a permanent unresolvable alarm. Exactly the failure mode D9 exists to prevent. The honest observation is SSH reachability, which is expensive, flaky, and conflates "reachable right now" with "manageable by design". **`sshAccess` stays `not-reported`** until it earns a real design.
- **Provenance — genuinely nothing live to compare.** `image` (the template vmid it was cloned from), `imageType` (clone vs download) and `autoInstall` (an unattended install that ran once) describe how the guest came to exist, not what it currently is. For these, D9 makes the *recorded* value the authoritative record, which is the best available answer; there is no actual to diff against and never will be.

So the six split **2 implemented / 1 deferred by design / 3 permanent**. The lesson generalizes past these fields: a *proxy* for a field is only worth reporting when it cannot invert the answer for some guest in the estate — a false actual on a class that never applies is strictly worse than not looking.

**Implementation requirement — the automatic disk grow must go through `modify`.** `health-manager/check-disk-threshold.sh` grows a guest's disk by 50% when `/` usage crosses a threshold (default 80%). It reads the deployed config only for `vmid` and `node`, computes the new size from the **live** `qm config` size, calls `controller/proxmox-controller/resize-disk.sh` directly, and **never writes `diskSize` back into the deployed config**. It bypasses `module-manager module modify` entirely.

That is already a latent bug, independent of D9. For a module that declares `diskSize` — nextcloud declares `80G` — an auto-grow to 120G leaves config at 80G against an actual of 120G. The converge reads that as a **shrink**, `update-disk.sh` refuses it (`exit 20`), and it refuses again on every subsequent converge, because nothing in the loop can ever move config forward. The drift is permanent and unresolvable without an operator. It has not bitten this cluster only because the auto-grow has never fired (there is no `logs/disk-resize.log`), and because a module declaring no size was, before D9, excluded from the comparison altogether. **Under D9 every module declares a size, so this stops being latent.**

The fix is the one D9 implies everywhere else: an automatic grow is a *declared-field change*, so it must be expressed as one — `module-manager module modify <m> --set diskSize=<new>`, which writes config and applies through the same gated converge as any other change, rather than a side-door mutation the change model cannot see. `check-disk-threshold.sh` then decides *when* to grow and stops being the thing that grows. This also makes the auto-grow subject to the disruption rules and auditable in the module's config history, which a direct `resize-disk.sh` call is not.

**Migration.** Modules deployed before this decision have incomplete configs, so a one-time pass is required to bring the estate to the post-install invariant. `adopt` over a module's owned, reportable fields is exactly that pass; `validate` should warn on a deployed config missing a value for an owned, reportable field until the estate is converted.
...
That was a symptom. The real gap is that the two managers built from this ADR took **opposite defaults** for an unclassified field. `network-manager` declares `changeClass` on all 17 fields of `zones-fields.json` and refuses one that has none — *"what changing it costs is unknown — declare one before it can be set through a verb"*. `module-manager` waves a field with no owning service through as `config-only` without consulting a class at all, which is how `--set kind=widget` or `--set location=/tmp` is accepted on a live module.
...
- **Positive.** One desired-state definition and one change-semantics home per service → validate/drift/reconcile/modify cannot silently disagree (the general form of #549/#550). Operators get a sanctioned `modify --set` for every declared field (#557) instead of hand-editing deployed config or reinstalling — over the *same* algorithm `update-tappaas` already trusts, so there is no second apply engine to keep in sync. The change taxonomy is declared and lint-checked, so a new provider states its field semantics instead of hiding them in an imperative loop, and `update-service.sh` handles a *changed* field correctly instead of by accident. The frame generalizes to `network-manager` (#538) and any future manager. ADR-019's `node` change becomes one hook in that frame rather than a special case. Under D9 the deployed config is **complete** after install, so drift is always a real post-install change and never an artifact of a schema default, and `validate` can check completeness rather than guessing.
- **Cost.** Real work: lift the resolver into `module-manager module resolve`; move normalize+diff into the manager and delete `inspect.ts`'s own `qm config` parsing; author `fields.json` + `report-service.sh` per existing service; split each `update-service.sh` into `--apply-drift` + hooks; add the `--set` pre-step, `rebootOk`, and the disruption/deferral plumbing. **Every existing `update-service.sh` must be migrated with behaviour preserved and its non-field logic retained (D7 migration discipline)** — the largest and most delicate part of the work, gated by keeping each script's existing tests green. **D9 adds work not covered by P0–P6:** install must write resolved values back into the deployed config, the `adopt` verb is new, the per-field opt-out and its skip reason come out of `lib/ts` and the manifests, and the existing estate needs a one-time adoption pass before `validate` can enforce completeness. Two further items surfaced with D9: `check-disk-threshold.sh`'s auto-grow must be re-expressed as `modify --set diskSize=…` instead of calling `resize-disk.sh` behind the change model's back, and `report-service.sh` learns the two `not-reported` fields that are genuinely observable (`os`, `cloudInit`); `sshAccess` stays unreported for want of a non-inverting observation.
...
All decided; the resolutions are folded into the decisions above and restated here for the record. Items 1–7 were settled in v0.3; 8–11 in v0.4 (realization); 12 in v0.6, after P0–P6 shipped and the flag it retires had been used in anger.
...
12. **The schema default is desired state for every field (D9).** Three shapes were weighed for "the module declares nothing". *Skip the field*, which v0.5 did through a per-field opt-out, is safe but leaves the field invisible for the life of the module. *Enforce a default nobody chose* manufactures drift that, for a `recreate` or `manual` class, can never be resolved. *Record at install what was actually used* keeps "I asked for 8G" distinct from "I never asked", makes config complete, and makes every later difference a real change worth reporting — so it was chosen. Working the estate's cases through one at a time showed the fear behind the opt-out was largely unfounded: for `vmtag`, `diskSize`, `storage` and `bios` alike, TAPPaaS's own creators build with exactly the schema default, so an undeclared guest already matches it. Only two residues needed real work — `bios` on a guest built elsewhere (closed by recording the observed firmware at install) and a disk grown outside the config path (closed by adopting rather than refusing). Rejected along the way: promoting the opt-out to a sixth **apply mode** (the fields carrying it spanned three different modes — `set`, `hook`, `none` — so a mode could not express both facts), and deriving it from the change class (it was not derivable: `in-place` 2 of 41, `manual` 3 of 4, `recreate` 1 of 4).
