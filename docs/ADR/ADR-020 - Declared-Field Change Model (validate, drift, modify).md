...
- **Disruption is authorized iff** `module modify <module> --force` was passed for that one module **OR** (`rebootOk == true` **AND** (the pass is scheduled with the site's `automaticReboot` on — `update-tappaas` exports `TAPPAAS_SCHEDULED_PASS=1`, the same signal that authorizes node reboots — **OR** the operator ran `site-manager update --force`)). A plain `site-manager update` opens no window, even with `automaticReboot` on (v0.9; ADR-017 v0.2 D8 lets the unit tell the two runs apart). `update-tappaas --force` is **never** forwarded as disruption authorization.
- **`site-manager update --force` runs every module's update now, past a failing pre-update test** (v0.8 #633, v0.9). Each `module modify` in that run gets `--ignore-test-failure`. It stands in for the maintenance window, never for a module's consent: a module with `rebootOk: false` keeps its disruptive changes deferred. Only `module modify <module> --force`, typed for one module, overrides `rebootOk: false`. (`update-tappaas --force` is deprecated by ADR-017 v0.2.)
- **`modify --force` authorizes disruption and bypasses the archived/external status skip; it does not override a failing pre-update test** (v0.8, #635). The pre-update gate follows the suite's own grading: exit 2 aborts, exit 1 proceeds with a `TEST-WARN:` line. The post-update test fails the module only on assertions that passed before the update. Overriding a fatal pre-update test takes its own flag.
...
Three distinct levers that no longer collide: `update-tappaas --force` / `site-manager update --force` (schedule: run now), `module modify <module> --force` (authorize disruption for that module now), `module.rebootOk` (standing per-module authorization, used by the scheduled pass and by a run-now).
