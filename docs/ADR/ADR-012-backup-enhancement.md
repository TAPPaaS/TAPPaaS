...
| **Author** | Lars Rossen |
| **Deciders** | @LarsRossen |
| **Related** | **#402** (flexible backup install on a cluster — origin); **#389** (remote/off-site backup setup + single-node); **#382** (adding a node does not install the backup client); **#456** (no placement policy for a pre-existing local PBS); **#214** (`pbsType`/external bare-metal PBS); **#501** (complement `dependsOn` with `integratesWith`); [ADR-010](ADR-010-vps-satellite-reverse-proxy-backup.md) (satellite off-site backup, pull model, compromise isolation); [ADR-007](<ADR-007 - TAPPaaS Taxonomy.md>) (named, not numbered foundation modules); [backup/QUICKREF.md](../../src/foundation/backup/QUICKREF.md) (PBS namespaces, multi-source pull/push — #227); **#544** (backup discovery scans `config/*.json` — misclassifies non-module state files); **#545** (backup + tested recovery for the foundation modules & `config/`) |
| **Implementation** | [ADR-012-implementation.md](../design/ADR-012-implementation.md) — plan, decisions log, package tracker |
| **Changelog** | v0.1 — skeleton + Context, three decisions drafted; expanded with symmetric peers, unified credentials, placement policy, bootstrap/promotion, manager/controller tooling; implementation tracker started (Lars, 2026-07-04). v0.2 — P1–P9 implemented: P1/P2/P3 live-verified on tappaas1; P4/P5/P6/P8 offline-green; P7 (TS layer) built+verified on cicd; P9 docs + live test plan (Lars, 2026-07-05). v0.3 — **restructured** the Decision into three sections — (1) supported backup topologies, (2) configuring/detecting the Backup module, (3) configuring clients & backups; a single `backup` module is always provisioned, only the **local** case installs PBS software (now clarified to run **directly on the cluster node's Proxmox OS, not a VM**); backup-buddy / cross-PBS **pull roles** given their own §1.4 (compromise invariant demoted to §1.4.1); **simplified the push/pull model** — only two movements exist (clients *push* to their configured PBS with write-no-delete creds; off-site buddies *pull*), so **no PBS→PBS push / "append-only push receiver" is needed** and immutability is reframed as datastore-at-rest hardening, not push safety; reworked placement into a single install-resolved **`placementState`** (ships empty → `node:<name>` or `shim`; install-forced `external` is permanent), **dropping the `placement` policy field** and merging `remote-only` into `external`; `pbsUrl` (default `backup.mgmt.internal`) is the target clients push to; introduced backup-type **capabilities** (`dependsOn: backup:vm` / `backup:filesystem` — no `type` field; module `provides` gains `filesystem`) + the **schedule cascade**; added a **Future improvements** section holding the deferred, underspecified pieces — at-rest immutability (`immutableSnapshots`), `userdata` export, and `dataset` backup; folded in **#456** (consume an externally-managed PBS by URL — satellite / external / local-external); moved the **workload-placement taxonomy** to [Appendix A](#appendix-a--workload-placement-taxonomy-companion--future-adr), destined for its own ADR; added **§2.7 module-schema changes** (exact `module-fields.json`/`backup.json` deltas — new `pbsUrl`, `backup.schedule`/`filesystemPaths`, **`provides` = `["vm", "filesystem"]`**, `pushTarget`/`alwaysBackup` deprecated) with an analysis recommending **retiring `alwaysBackup`** once #501 adds an `integratesWith` relationship (backup stays opt-in — hardware/test modules take none); added **§4 migration** (existing-TAPPaaS upgrade, #456 adoption, datastore relocation-by-pull, non-PBS) with its own plan; and made the implementation plan **explicitly include documentation** updates. No decisions reversed; P1–P9 status unchanged (Lars, 2026-08-29). v0.4 — §1 names a **fourth supported shape**: a local PBS on a **Site-managed host that is not a cluster member**, which the reference cluster-node case now generalises; `placementState: node:<name>` becomes **`host:<name>`**, since the value named a cluster node it could not guarantee (§2.1, §2.2, §2.3, §4.1); §2.2 states that `backup.json.node` may name a non-member host and that discovery must not depend on a Proxmox-only probe; §1 records the **known limitation** that such a host's OS is not yet in the update sweep. No decision reversed — the reference topology is unchanged (#599, #600). v0.5 — placement state **`external` → `consumed:<url>`**, named for what the module does rather than who owns the target, and **§2.3 gains its exit** (explicit install-time override, gated on §4.3's pull + test restore) (#606, #607); **§2.7C states that `services/{remote,external,push}` are unaffected** — dropping the `provides` entries removes capabilities, not the runtime peer wiring (#608); **§1.4 requires an off-site target to declare a `location.country` differing from the Site's**, with `satellite-fields.json` gaining the field and Testing asserting it, so the 3-2-1 claim is checkable rather than assumed (#609); **Acceptance split into design and implementation**, the live suite referenced to #407 instead of duplicated, since one list mixing "agreed" with "built" could never complete (#605). No decision reversed. |
...
Earlier placement covered a PBS the module *installs* on a node (a `node:<name>` state) and a PBS that *doesn't exist yet* (a `shim`). Neither covers a site that **already runs its own PBS on the local network**, provisioned outside TAPPaaS: it has a real local datastore, so `shim` doesn't apply, and the module didn't place it, so a `host:<name>` state would describe work that never happened. The module needs a way to *consume* such a PBS — given its address (URL), register Proxmox storage and drive jobs/clients — without discovering storage or installing anything. (This ADR resolves it with the `external` state, §2.1.)
...
| **Local PBS on a Site-managed host** (§1.2) | **yes** — installed on that host's OS (not a VM) | yes | state `host:<name>` — a datastore pool is found on `<name>` | reference 2–3-node cluster; also a Site-managed host outside the cluster |
| **Externally-managed PBS** (§1.3) | no — **consumed** by URL | depends on flavor | state `consumed:<url>` — operator names a PBS to consume | single-node, off-site-only, or a site that already runs PBS (#456) |
...
> **Known limitation — a local PBS on a host outside the cluster.** The module installs, discovers
> and drives such a host exactly as it does a cluster member, but the host's **own OS is not yet
> covered by the update sweep**: `update.sh` reconciles backup *clients* across cluster membership,
> and `health-manager update-os` addresses a guest by `<name> <vmid> <node>`, which a host with no
> vmid and no cluster membership cannot satisfy. Until that is closed the host is patched by hand.
> Tracked separately as application-lifecycle work, out of scope for this ADR.
...
### 1.2 Local PBS on a Site-managed host (module-provisioned)

The module **discovers** a datastore pool (configured host first, then any cluster member — see §2.2) and installs **PBS software directly on that host's OS** — via the Proxmox PBS package, it is **not** a separate guest VM — and owns the datastore on that host's `tankc` pool. This is the reference topology, now conditional on a pool being present.

The host **may or may not be a Proxmox cluster member**. A cluster member is the reference case; a Site-managed host outside the cluster — bare metal running PBS, still installed and updated by this module — is the same topology and takes the same state. Cluster membership is a property of the host recorded in `site.json`, not a property of the placement. The datastore it creates is the local backup target for the site's guests (§3).
...
*State name: `consumed:<url>`. The three flavours below differ only in **reachability** — tunnel, public network, LAN. They do not share an ownership meaning: a satellite is provisioned and managed by this Site, a third party's PBS by nobody here, a buddy's by another Site. The state records what the module does with the target — register the URL, provision nothing — not who runs it.*
...
| `consumed:<url>` | **install told to consume an existing PBS** (+ a `pbsUrl`) | register the PBS at that URL as storage; provision nothing. **Sticky — see §2.3 for the exit.** | §1.3 |
...
> **Naming note (open):** #214 introduced a `pbsType`/`external` notion for bare-metal PBS. How the install-time "consume this PBS" is spelled (a dedicated flag, `pbsType`, …) is an implementation detail for the tracker — the **decision here** is: no `placement` policy field; `placementState` ships empty and is install-resolved; `consumed:<url>` is set at install (with `pbsUrl`) and is sticky.
>
> The state is **not** called `external`. That word already carries the ADR-012 Appendix A sense ("tracked but not managed by this Site"), the `kind: external-host` sense, and the `external/<name>` PBS namespace — and a satellite consumed this way *is* managed by this Site, so naming the state for ownership misdescribes the commonest case. `consumed:` names what the module does.

### 2.2 The resolution order

At install/update, `install.sh` resolves `placementState`:

1. **Told to consume an existing PBS?** → `placementState = consumed:<url>` (requires `pbsUrl`). No discovery, nothing provisioned. Sticky thereafter — deliberate operator intent is not re-derived away.
2. Already a concrete state (`host:<name>` or `consumed:<url>`) → **keep it** (idempotent).
...
| `placementState` | **change values + ships empty** | The single source of truth (§2.1). Was `local\|shim\|remote-only`; now **empty (released default)** or pattern `^(shim\|consumed:.+\|host:.+)$`. Install-written, never hand-authored. Migration: legacy `local` → `host:<name>`, `remote-only` → `consumed:<url>`. |
| `pbsUrl` | **new** | The PBS the clients push to (§1.4). **Default `backup.mgmt.internal`** (local PBS DNS); overridden to the consumed PBS's URL when `placementState: consumed:<url>` (§1.3). Credential prompted-not-stored. |
| `pushTarget` | **deprecate** | Subsumed by `placementState: consumed:<url>` + `pbsUrl` — the consumed PBS is simply the configured target clients push to. Read for one release, then removed. |
...
**Discovery & the foundation gap (#544, #545).** Two issues fell out of this design and must be resolved with it. **#544** — `backup-manager`'s target discovery scans `config/*.json` behind a stale deny-list and misclassifies non-module state files (`last-update-result.json`, `zones.effective.json`, …), so it must instead reuse `module-manager`'s module discovery and filter to `dependsOn: backup:vm | backup:filesystem` (consistent with `moduleInPbsJob`). **#545** — with `alwaysBackup`/`integratesWith` still unresolved, the four foundation modules (`cluster`, `network`, `templates`, `tappaas-cicd`) sit in no PBS job; #545 decides which of them need backup at all, whether `config/` (the cluster's source of truth) should be a **`backup:filesystem`** capture rather than a full VM backup, and — critically — defines a **written, tested recovery path** for the firewall, the mothership and the config. A backup with no rehearsed restore is not a backup.
...
- `placementState:local` (or empty) → `host:<name>`, where `<name>` is the host currently serving PBS, cluster member or not — **the datastore is left exactly where it is**, no move, no dependent reinstall.
- `placementState:remote-only` → `consumed:<url>`, synthesising `pbsUrl` from the old push-target config.
...
2. **`#456` adoption path** — install-time `consumed:<url>` + `pbsUrl` registers an existing PBS as storage without provisioning; verify existing snapshots remain restorable (§4.2).
...
- **More placement states to reason about** (`host:<name>` / `shim` / `consumed:<url>`) and to test.
...
1. **Placement resolution + shim (#402)** — resolve `placementState` from the `external` force / `node` discovery-constraint inputs in `backup.json`; make `install.sh` discover a pool (the pinned `node` first if set, else cluster members), install PBS on the host where found (→ `host:<name>`), or lay down a flagged shim with a warning (→ `shim`); record the resolved state idempotently.
...
8. **Bootstrap & promotion (§2.3)** — wire placement resolution into `install.sh`; make `update-module.sh backup` promote a shim to `host:<name>` / `external` / host+satellite without dependent reinstall.
9. **Hardening (#389)** — the compromise-isolation test suite.
10. **Consume a pre-existing PBS (#456)** — add the install-time `consumed:<url>` placement state (URL via `pbsUrl`: satellite / external / local-external); register consumed storage + jobs + client rollout without discovering storage or installing PBS. *(new — v0.3)*
...
15. **Documentation (all changes)** — update `backup/README.md`, `QUICKREF.md`, `TEST.md`; the module-authoring guide ([`apps/00-Template`](../../src/apps/00-Template/)) for the backup `dependsOn` capabilities + `backup.schedule` + placement states; the migration + key export/import runbooks; and this ADR + its [implementation tracker](../design/ADR-012-implementation.md). *(new — v0.3)*

## Testing Strategy

- **Placement:** with a pool → PBS on the right host; **a configured host that is not a cluster member and serves PBS resolves to `host:<name>`, not to a cluster member and not to a shim**; no pool → a **shim** (no VM), a warning, and a `dependsOn: backup` module still installs; adding `tankc` + re-running `update.sh` **promotes** the shim and the dependent module still works.
- **Consume pre-existing PBS (#456):** install-time `consumed:<url>` + a `pbsUrl` → the module registers storage + jobs and rolls out clients **without** discovering storage or installing PBS; a `dependsOn: backup` module backs up to the consumed datastore.
...
- **Bootstrap/promotion (§2.3):** `shim → external` and `shim → host:<name>` (and `→ host+satellite`) via a config change + `update.sh` re-resolving `placementState`; dependents keep working throughout.
...
- **Migration (§4):** a legacy fixture (`placementState:local`, `pushTarget`, `alwaysBackup`) upgrades to `host:<name>` with the datastore untouched, **including when `.node` names a host that is not a cluster member**; a `#456` `external` adoption leaves the pre-existing snapshots restorable; a relocation-by-pull preserves history and only decommissions the old datastore after a test restore.

## Acceptance

Two lists, because they answer different questions. **Design acceptance** is whether the
decision is agreed — that is what ADR-013 means by *Accepted*, and none of it needs hardware.
**Implementation acceptance** is whether the code exists and has run; ADR-013 calls that state
*Accepted — implemented*, and the live suite is tracked in **#407**, not duplicated here.
Mixing the two is why this list could not be completed.

### Design acceptance — the decisions this ADR makes

- [x] **Supported topologies (§1)** — shim · local PBS on a Site-managed host (cluster member or not) · externally-managed PBS consumed by URL.
- [x] **Two movements only (§1.4)** — clients push to their configured PBS; off-site buddies pull. No PBS→PBS push exists, so there is no source→destination write path to harden.
- [x] **Compromise isolation (§1.4.1)** — write-no-delete client credentials with PBS-owned prune, plus pull-only inter-PBS copies. Cross-domain trust follows RFC 1136: components inside one administrative domain trust each other, across domains they do not.
- [x] **Off-site is recorded, not asserted (§1.4)** — an off-site target declares a `location.country` differing from the Site's.
- [x] **Placement is a resolved state, not a policy (§2.1)** — `host:<name>` · `shim` · `consumed:<url>`; install-written, never hand-authored; the exit from a sticky `consumed:` is documented (§2.3).
- [x] **Credential model (§2.5)** — exactly two shapes, read-only pull token and write-no-delete backup login, identical for every peer type. No secrets in config.
- [x] **Key escrow and DR (§2.5.1)** — central escrow plus a mandatory out-of-band copy; export/import to removable media is a first-class path.
- [x] **Manager / controller split (§2.6)** — `backup-manager` owns policy, `backup-controller` owns live PBS state and is endpoint-agnostic.
- [x] **Backup kind is a capability, not a field (§3.1)** — `dependsOn: backup:vm` / `backup:filesystem`; declaring neither is how a module opts out.
- [x] **Schedule cascade (§3.2)** — Site → Environment → Module, with a once-per-day ceiling.
- [x] **Migration model (§4)** — four starting points, all reusing state re-resolution and buddy pull; no new machinery.
- [x] **Deferrals are explicit (Future improvements)** — at-rest immutability, `userdata` and `dataset` are named, not silently omitted, and §1.4.1 does not depend on them.

### Implementation acceptance — what is built and verified

- [x] `install.sh` resolves placement, else lays down a flagged shim. *(#402)* — **live-verified on tappaas1** *(as the `auto`/`node:`/`shim`/`remote-only` policy enum; the state rename is not yet in code)*
- [x] Shim → real-PBS **promotion** via `update-module.sh backup`, `dependsOn: backup` consumers preserved. *(#402)* — **live-verified**
- [x] Per-node client install is an **idempotent reconcile** owned by `update.sh`. *(#382)* — **live-verified (single node)**
- [x] Off-site path with a **write-no-delete** client credential and remote-owned retention. *(#402, #389)* — offline-green *(coded as `remote-only`)*
- [x] Off-site **subset** selection + **independent retention**. *(#389)* — implemented
- [x] Any PBS works as pull source, pull destination and client-backup target, on one credential flow. *(§1.4/§2.5)*
- [x] **PBS-endpoint-agnostic** tooling — `backup-manager`/`backup-controller` honour `--pbs`. *(§2.6)* — **built + verified on cicd**
- [x] Placement discovery finds a PBS on a host that is not a cluster member. *(#601)* — unit-tested, mutation-verified
- [x] `QUICKREF.md` / `TEST.md` updated (v0.2 baseline).
- [ ] **Placement state set in code** — drop `placement`; `placementState` ships empty; `host:<name>` / `consumed:<url>`.
- [ ] **Consume a pre-existing PBS (#456)** — install-time `consumed:<url>` registers storage + jobs + clients without provisioning.
- [ ] **Backup-type capabilities (§3.1)** — `backup:filesystem` added; membership via `dependsOn`; opt-out honoured.
- [ ] **Schedule cascade (§3.2)** in code, with the once/day ceiling.
- [ ] **Module-schema changes (§2.7)** — `pbsUrl`, `backup.schedule`/`filesystemPaths`, `provides`, deprecations, KI-1 normalizer fix.
- [ ] **`alwaysBackup` retired** via `integratesWith`. *(#501 has landed — this is now buildable, not blocked)*
- [ ] **Migration (§4)** — legacy backfill, #456 adoption, relocation-by-pull.
- [ ] **Documentation (§Impl 15)** — module docs, `00-Template`, migration and key export/import runbooks.
- [ ] **Live suite on the 3-node cluster** — compromise isolation, restore with and without the key, live push. **Tracked in #407**; not restated here.

> **On #407.** It states its purpose as advancing this ADR *draft → proposed*, but the status
> reached **Proposed** on 2026-09-02 by an operator sign-off recorded outside it. Whether #407
> now gates *proposed → accepted* is an open question for the deciders — and note that every
> other open ADR-012 issue is milestoned Release 2.0 while #407 is 2.1.

---

## Appendix A — Workload placement taxonomy (companion — future ADR)

> **Status:** companion reference, **not** a decision of this ADR. Earlier ADR-012 drafts carried this inline (as §1.2); it is a *broader* concern than backup — it classifies **any** workload TAPPaaS is aware of, not just PBS placement — and is parked here to graduate into **its own ADR**. It does **not** replace the `host:<name>`/`shim`/`external` placement states (§2.1), which stay as the implemented mechanism.
