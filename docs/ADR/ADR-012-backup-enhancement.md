...
- Run **`backup-manager placement use-external <url>`** (as built, #456). It prompts for the credential the remote PBS issues (never stored, §2.5), **registers it as Proxmox storage**, verifies the existing contents are listable, and records `placementState: external` + `pbsUrl`; the module then rolls out per-node clients (§2.4) and the managed job — **without creating a datastore or touching the PBS's existing contents**. Forcing the state alone (`module modify --set placementState=external`) does not register the storage, which needs the prompted credential; every update of an `external` placement now **fails, named**, when the storage is not registered or not usable, instead of reporting success while no backup can land.
...
- [x] §2.2 rule 3 in `install.sh` and `update.sh`: probe for a serving PBS before any discovery (#602) — on `main`, live-verified on hrossen and, on a machine Host, in the debianhost test plan phase 6.
- [x] `backup-manager placement reset` (#607) — and `finish-reset`. Unit-tested (the order, every refusal); the storage rename live-verified on the reference cluster on a throwaway copy of the real entry (195 snapshots visible through `_former`). **Rehearsed end to end 2026-09-19** (#407 suite): use-external → reset → local PBS → pull of the old history → test restore → finish-reset; it found and fixed a substring storage check that would have left a real reset with no backup storage.
...
- [x] The code writes `placementState: node` with the Host in `backup.json.node`, as §2.1 decides, instead of `node:<name>`; migration 0007 rewrites existing sites (#600) — built 2026-09-19. Readers accept both shapes for one stable cycle; `node` naming no Host reads as unresolved. The 3-way merge keeps the Host in `.node` (proved in the 0007 fixture test against the real merge).
...
- [x] Per-node client install is an **idempotent reconcile** owned by `update.sh`; a node added later gets its client. *(#382)* — **live-verified (single node)**. On Proxmox VE 9 the client is a hard dependency of Proxmox itself, so a new node always has it; the reconcile keeps the PBS repository in place.
- [x] **External / no-local-PBS** off-site path implemented with a **write-no-delete** client credential and remote-owned retention. *(#402, #389)* — **live 2026-09-19**: a push to a second PBS with a `DatastoreBackup` login; forget and prune with that login refused.
...
- [x] **Consume a pre-existing PBS (#456)** — `backup-manager placement use-external <url>` (the operator verb, 2026-09-19; it runs `backup-manage.sh use-external`). **Live-verified**: consumed the site's PBS by URL under a throwaway storage name, 165 pre-existing backups visible and restorable, nothing created or modified.
...
- [x] **Migration (§4)** — state backfill live-verified (no datastore move, no dependent reinstall) and unit-tested against legacy fixtures; #456 adoption live-verified to preserve snapshots. Relocation-by-pull **rehearsed 2026-09-19**: a VM's full history (21 snapshots) pulled into a second PBS and test-restored from there.
...
- [x] Restore proven **with** the key and refused **without** it — live, on the mothership's `config/` capture (`missing key - manifest was created with key …`). The key's out-of-band export/import round trip is proven too. **From an off-site copy too (2026-09-19):** the encrypted capture pulled to a second PBS restored with the key and was refused without it.
