...
| **Version** | 0.4 |
| **Date** | 2026-08-29 (v0.4: 2026-09-09) |
| **Author** | Lars Rossen |
| **Deciders** | @LarsRossen |
| **Related** | **#402** (flexible backup install on a cluster — origin); **#389** (remote/off-site backup setup + single-node); **#382** (adding a node does not install the backup client); **#456** (no placement policy for a pre-existing local PBS); **#214** (`pbsType`/external bare-metal PBS); **#501** (complement `dependsOn` with `integratesWith`); [ADR-010](ADR-010-vps-satellite-reverse-proxy-backup.md) (satellite off-site backup, pull model, compromise isolation); [ADR-007](<ADR-007 - TAPPaaS Taxonomy.md>) (named, not numbered foundation modules); [ADR-022](<ADR-022 - Workload Ontology.md>) + [ADR-023](<ADR-023 - Workload Classification.md>) (the workload vocabulary and classification this ADR's Appendix A graduated into — draft, with two points open against §2.1); [backup/RESTORE.md](../../src/foundation/backup/RESTORE.md) (recovery per scenario) and [backup/README.md](../../src/foundation/backup/README.md) (PBS namespaces, multi-source pull/push — #227); **#544** (backup discovery scans `config/*.json` — misclassifies non-module state files); **#545** (backup + tested recovery for the foundation modules & `config/`) |
| **Implementation** | [ADR-012-implementation.md](../design/ADR-012-implementation.md) — plan, decisions log, package tracker |
| **Changelog** | v0.1 — skeleton + Context, three decisions drafted; expanded with symmetric peers, unified credentials, placement policy, bootstrap/promotion, manager/controller tooling; implementation tracker started (Lars, 2026-07-04). v0.2 — P1–P9 implemented: P1/P2/P3 live-verified on tappaas1; P4/P5/P6/P8 offline-green; P7 (TS layer) built+verified on cicd; P9 docs + live test plan (Lars, 2026-07-05). v0.3 — **restructured** the Decision into three sections — (1) supported backup topologies, (2) configuring/detecting the Backup module, (3) configuring clients & backups; a single `backup` module is always provisioned, only the **local** case installs PBS software (now clarified to run **directly on the cluster node's Proxmox OS, not a VM**); backup-buddy / cross-PBS **pull roles** given their own §1.4 (compromise invariant demoted to §1.4.1); **simplified the push/pull model** — only two movements exist (clients *push* to their configured PBS with write-no-delete creds; off-site buddies *pull*), so **no PBS→PBS push / "append-only push receiver" is needed** and immutability is reframed as datastore-at-rest hardening, not push safety; reworked placement into a single install-resolved **`placementState`** (ships empty → `node:<name>` or `shim`; install-forced `external` is permanent), **dropping the `placement` policy field** and merging `remote-only` into `external`; `pbsUrl` (default `backup.mgmt.internal`) is the target clients push to; introduced backup-type **capabilities** (`dependsOn: backup:vm` / `backup:filesystem` — no `type` field; module `provides` gains `filesystem`) + the **schedule cascade**; added a **Future improvements** section holding the deferred, underspecified pieces — at-rest immutability (`immutableSnapshots`), `userdata` export, and `dataset` backup; folded in **#456** (consume an externally-managed PBS by URL — satellite / external / local-external); moved the **workload-placement taxonomy** to [Appendix A](#appendix-a--workload-placement-taxonomy-graduated--adr-023), destined for its own ADR; added **§2.7 module-schema changes** (exact `module-fields.json`/`backup.json` deltas — new `pbsUrl`, `backup.schedule`/`filesystemPaths`, **`provides` = `["vm", "filesystem"]`**, `pushTarget`/`alwaysBackup` deprecated) with an analysis recommending **retiring `alwaysBackup`** once #501 adds an `integratesWith` relationship (backup stays opt-in — hardware/test modules take none); added **§4 migration** (existing-TAPPaaS upgrade, #456 adoption, datastore relocation-by-pull, non-PBS) with its own plan; and made the implementation plan **explicitly include documentation** updates. No decisions reversed; P1–P9 status unchanged (Lars, 2026-08-29). v0.4 — **Status Draft/Proposed → Accepted: implemented.** No decision changed; this records that the design was built and exercised. P10–P20 delivered the v0.3 scope — placement as an install-resolved state (`placement` field removed, `external` + `pbsUrl` added), `backup:filesystem`, the schedule cascade realised as one job per frequency, `alwaysBackup` retired via `integratesWith` (#501), shape-based discovery (#544), the node-add reconcile (§2.4), #545 coverage with a **rehearsed** recovery runbook, and key export/import (§2.5.1). Live testing found five defects no offline test could reach — most seriously that the mothership had never been in the backup job at all, and that `restore.sh` reported success while restoring nothing. Two implementation notes correct the ADR text where reality differed: §2.7 lands in the module’s own field manifests (since #567 `schemas/module-fields.json` holds only generic fields), and `placementState` tolerates the legacy `local`/`remote-only` for one release so deployed configs validate before their migrating update runs (Lars, 2026-09-09). v0.4a — §2.7C gained the **service-directory disposition table** (#608): `provides` and `services/` are deliberately no longer a 1:1 mirror, so the ADR now says which directories are kept, which are new, and that the peer directories (`remote`/`external`/`push`) remain the supported operator path even though their names left `provides`. Also records that deprecating `pushTarget` does not affect `services/push/`, which never read it (Lars, 2026-09-09). v0.5 — **peer vocabulary settled and made symmetric** (§1.4, operator review of #608). The three relationships another PBS can have with ours are now named for what they are: **`pull`** (we copy theirs), **`remote`** (they copy ours — where our off-site copies live), **`receive`** (they push theirs into ours, having no PBS). `remote` is **new**: the symmetric half of `pull` had no implementation, only prose telling an operator to issue a token by hand. The old `push` peer — this cluster sending vzdump backups to a remote PBS — is **retired**: that case is `placementState: external` + `pbsUrl`, a placement rather than a peer, and keeping both was the source of the word "external" meaning two opposite things. Config files and namespaces follow the verbs (`pull-<n>`/`pull/<n>`, `remote-<n>`, `receive-<n>`/`receive/<n>`); no migration, as no deployment had a peer configured. `peer delete` requires the kind, because one name legitimately holds two relationships at once — a buddy is usually both a `pull` and a `remote` (Lars, 2026-09-09). v0.5a — **Appendix A retired into [ADR-023](<ADR-023 - Workload Classification.md>)**, which is the ADR it was always destined to become. The six-value table is not reproduced here: two copies of a classification drift, and the stale one is the one someone eventually acts on. Appendix A is now a pointer, and records the two places ADR-023's draft would change something this ADR has already built and live-verified — `kind` acquiring a second, orthogonal meaning alongside the `kind: "module"` object-type marker that module discovery reads (#544), and `shim` becoming `realized: false` rather than a `placementState` value a live cluster is already migrated onto. Raised, not decided; both ADRs are drafts under review (Lars, 2026-09-09). |
...
| Kind | What it is | Who initiates | Credential this side holds | Namespace here |
|---|---|---|---|---|
...
| **`receive`** | they *push* their backups into ours, having no PBS of their own | they do | none; we *issue* them a write-no-delete login | `receive/<name>` |
...
| `services/push/` | **retired** | it sent this cluster's vzdump backups to a remote PBS — the case `placementState: external` + `pbsUrl` now owns (§1.3). Keeping both was why "external" meant two opposite things |
| *(new)* `scripts/remote/` | **added** | the symmetric half of `pull`: granting a peer read-only access so they can pull **our** backups (§1.4) |
...
- The workload-placement taxonomy is **not** a decision of this ADR: it graduated into [ADR-023](<ADR-023 - Workload Classification.md>), and [Appendix A](#appendix-a--workload-placement-taxonomy-graduated--adr-023) points there. Two of ADR-023's draft decisions (`kind`'s meaning, and `shim` as a state) touch what this ADR implemented, and are listed there as open.
...
`external` · `remote` · `rogue` — are retired in ADR-023 by splitting the one
...
ours, **what it runs on** (`kind`: `guest` / `host`). All six survive as
coordinates in that model; none is lost. See ADR-023's mapping table.

**What this ADR still decides**, and what ADR-023 does not touch: the backup
...
### Open against ADR-023 (both drafts, raised not decided)

Two points where ADR-023 as drafted would change something ADR-012 has already
...
1. **`kind` would carry two orthogonal meanings.** In the code today `kind` is
   ...
   JSON in `config/`. ADR-023 redefines `kind` as *what the workload runs on*
   (`guest` / `host`), retiring `external-host`. Those are answers to different
...
2. **`shim` as `realized: false`.** ADR-023 §3 argues `shim` is a *state* of a
