...
| [ADR-027 — Module Blueprint](<ADR-027 - Module Blueprint.md>) | Draft | What every module is made of: the artifact set with MUST/SHOULD/MAY on each, all of it co-located in the module's directory, the claim its own JSON carries (#248), and a blueprint check whose severity follows tier and source. `00-Template` is the blueprint in files. Five questions open for the deciders. |
