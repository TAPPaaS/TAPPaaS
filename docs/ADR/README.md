...
| [ADR-012 — Backup enhancement](<ADR-012-backup-enhancement.md>) | Accepted (v1.0) — v1.0 additions not built yet | Where PBS lives (install-resolved placement, or an external PBS consumed by URL), what a module asks to have backed up (`backup:vm` / `backup:filesystem`), and how often (site → environment → module cascade). |
...
| [ADR-027 — Module Blueprint](<ADR-027 - Module Blueprint.md>) | Proposed | What every module is made of, apart from its documents: the executable artifact set with MUST/SHOULD/MAY on each, the six files of a `services/<service>/` directory, all of it co-located in the module's directory, and one blueprint check for every module plus a sweep to make the estate meet it. Documents are ADR-013's, contribution files ADR-015's. `00-Template` is the blueprint in files, and its stubs warn that they are stubs. |
| [ADR-028 — OS Version Tracking](<ADR-028 - OS Version Tracking.md>) | Proposed | Which operating-system bits a TAPPaaS system runs, when security patches arrive and when a major moves. NixOS guests are **built, not upgraded**: they hold no flake and no channel, and `update-os.sh` forces the revision from one lock, so they patch only when a human moves that file — while Debian guests, managed machines and the (Debian) satellite patch themselves every sweep — by three different apt paths. One estate pin (`templates/flake.lock`, with the mothership's flake following it) replaces two that can diverge; two rhythms (monthly lock refresh, branch move within a month of each NixOS release); the ref stays a Hydra-built `nixos-*` channel so guests substitute instead of compiling; and the #680 pin-age tripwire tightens to 45 days, which also detects branch EOL for free. |
...
| [ADR-015 — Community Governance and Contribution Files](<ADR-015 - Community Governance and Contribution Files.md>) | Draft | The community-health file set (CONTRIBUTING, CODE_OF_CONDUCT, SECURITY, GOVERNANCE, CODEOWNERS, templates) — names, per-repo placement, and contents, plus the per-module `AUTHORS.md`. The companion to ADR-027, which defers every contribution file here. |
