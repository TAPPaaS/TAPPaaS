...
> **⚠ Reversed during implementation (impl-doc D19/Q8, 2026-07-03).** The satellite ships **Debian 12/13 by default** (`satellite-manager install … --os debian`), **not** NixOS. Reasons: it runs the vault on official `proxmox-backup-server` (no unofficial Nix/OCI PBS port — see §3.4), bootstraps trivially (Hetzner boots Debian; no `nixos-anywhere`), and gives the vault **OS diversity** — a nixpkgs/`nixos-anywhere`/community-flake supply-chain compromise hits the (NixOS) cluster but **not** the off-site vault (strengthens §7.3). The declarative **NixOS path is retained** as `--os nixos` (`satellite.nix` + `nixos-anywhere`). The NixOS-centric wording in §5.2–§5.5 below describes that optional path.

The satellite is deliberately **minimal**: a WireGuard endpoint, an SNI-aware TCP forwarder (the passthrough relay, §2), and — when the backup role is enabled — an official PBS datastore target (§3). No application workloads (non-goal). On Debian the config is provisioned declaratively over SSH (`provision-debian.sh`) and kept current with `unattended-upgrades` (security-only + reboot window); the `--os nixos` path keeps the whole state reproducible from the repo via `autoUpgrade`. Either way the node is rebuildable from scratch after a provider loss (it holds no app state).
