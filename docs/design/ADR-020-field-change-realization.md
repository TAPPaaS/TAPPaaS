...
| `site-manager update [--force]` | **"run the sweep now"**; never forwards `module modify --force` (`--force` passes a failing pre-update test instead). `update-tappaas --force` is deprecated (ADR-017 D5) |
