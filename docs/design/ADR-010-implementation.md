...
| D10 | **Provisioning:** Hetzner **Tier A (portal allocate) = default**, Tier B (`hcloud` API token) opt-in. **Satellite OS = Debian 12/13 by default (`--os debian`, Option 3 — 2026-07-03, see D19)**, provisioned over SSH (no `nixos-anywhere`); the declarative NixOS path (`satellite.nix` + `nixos-anywhere`) stays available via `--os nixos`. | ADR §5.3; Q8 |
...
| D13 | **Compromise isolation, applied uniformly to every satellite:** no standing cicd root; one-directional management over the tunnel; Hetzner token never standing; **immutability via the PULL model** (satellite owns prune/GC; home holds no delete credential over the vault) — **NOT S3 Object Lock**, which PBS does not support (see D16). Auto-patching is OS-appropriate (Debian `unattended-upgrades` security-only + reboot window; NixOS `autoUpgrade`). | ADR §7.3 |
...
| D16 | **CORRECTED 2026-07-03.** Backup storage backend = local ZFS datastore on the satellite by default (PBS 4.2 S3 datastore is an option). **The 2026-06-30 "immutability via S3 Object Lock" premise is WITHDRAWN: PBS does not support S3 Object Lock/versioning — enabling it corrupts the datastore** ([Proxmox Bugzilla #6780](https://bugzilla.proxmox.com/show_bug.cgi?id=6780)). Immutability is delivered by the **pull model** (D8/D13): the satellite pulls from home and owns prune/GC, so a compromised home cannot delete the off-site copies. Out-of-PBS Object-Lock copies remain a possible future add-on. | ADR §3.4, §7.3; Q8 (2026-07-03) |
...
| D19 | **Satellite OS = Debian by default (Option 3, resolves Q8 — 2026-07-03).** Runs the vault on an officially-supported base (official `proxmox-backup-server`, no unofficial Nix/OCI PBS port), makes bootstrap trivial (Hetzner boots Debian; no `nixos-anywhere`), and — the deciding factor — gives **OS diversity**: a nixpkgs/`nixos-anywhere`/community-flake supply-chain compromise hits the cluster but **not** the off-site vault (strengthens §7.3). The tunnels port cleanly (WireGuard/nginx-stream/nftables are all native Debian), so one Debian satellite carries all roles — no combined-node OS split. Supersedes the "uniform NixOS" assumption in D10/D13; `--os nixos` remains for the declarative path. | Q8 stage log (2026-07-03) |
...
| **P6** | backup role (PBS **pull** + client-side encryption; immutability via pull model, NOT Object Lock) | TBD | P2, P3, **ADR-007 S9** (backup-manager/controller) | 🧪 body implemented (**Q8→D19**) | `provision-backup.sh` (official PBS install + local datastore + `remote` + pull `sync-job` `--remove-vanished false` + destination prune) + `edge->home-PBS:8007` rule + AllowedIPs widening + home read-only-token runbook. **Not yet live-tested** (needs a home PBS + a real Debian satellite). | — | — |
...
- **Option 4 is withdrawn (fatal).** It relied on **S3 Object Lock**, but **PBS does not support Object Lock/versioning at all — and enabling it on the bucket *corrupts* the datastore** (locked chunks block GC). Upstream: [Proxmox Bugzilla #6780](https://bugzilla.proxmox.com/show_bug.cgi?id=6780); confirmed independently ([WZ-IT, 2026-05](https://wz-it.com/en/blog/proxmox-backup-server-4-2-s3-storage-stable/); Proxmox forum, staff). **This also corrects D16** — Object Lock was never a valid immutability mechanism. Immutability therefore comes from the **pull model** (D8: satellite pulls + owns prune/GC), which means the satellite **must** run PBS.
- **Option 1 (native-Nix PBS)** — only real effort is [`AWildLeon/nixos-pbs`](https://github.com/AWildLeon/nixos-pbs) (created 2026-06-23, builds PBS 4.2.2 via `buildFHSEnv`, real `services.proxmox-backup-server` module). But: ~2 weeks old, 3★, self-labeled experimental, **no independent public tester success**, known footguns (datastore under `/` deleted), S3 backend untested, single-maintainer FHS-shim upstream-tracking burden. **Too immature for the vault.**
- **Option 2 (OCI PBS on NixOS)** — one viable image, [`ayufan/proxmox-backup-server`](https://github.com/ayufan/pve-backup-server-dockerfiles) (500k+ pulls, tracks 4.2, self-"unmaintained"). Overhead **negligible** (namespaces/cgroups; no systemd-in-container; datastore = host ZFS bind-mount — never NFS/overlay). Works in homelabs. Risks: unsupported config, single-maintainer supply chain on the vault, FUSE-restore/tape/PAM weak. Pull-sync works fine in a container. Pragmatic fallback if declarative-NixOS were mandatory.
- **✅ Option 3 (Debian satellite) — CHOSEN (D19).** Official `proxmox-backup-server`; trivial bootstrap (Hetzner boots Debian; no `nixos-anywhere`); and the deciding factor — **OS diversity protects the vault from a nixpkgs supply-chain compromise** (§7.3). The tunnels port cleanly (WireGuard/nginx-stream/nftables all native Debian), so one Debian satellite carries **all** roles.

**Implemented in this branch (`ADR010-debian-satellite`):** `--os debian|nixos` selector (default `debian`); `satellite/debian/provision-debian.sh` (idempotent, role-gated on-host installer); `lib/provision.sh` renders the Debian configs (`wg-infra.conf`, `nftables.conf`, `nginx-stream.conf`, ip-forward, `unattended-upgrades`) from the same `SAT_*` values + `sat_provision_debian` (SSH deploy, no reformat); schema `os` field; `satellite/debian/README.md`. **Backup body implemented (2026-07-03):** `satellite/debian/provision-backup.sh` installs the **official** `proxmox-backup-server` (Proxmox `pbs-no-subscription` repo, same source as home), creates the local datastore, and wires the pull: `remote` → home PBS + `sync-job` (`--remove-vanished false`, into `remote/<name>`) + a destination-owned prune-job (retention lives on the satellite → immutability). `satellite-manager` renders `backup.env` (`sat_gen_backup_config`), ships the home read-only **token** out-of-band (`TAPPAAS_SAT_PBS_TOKEN[_FILE]`, `0600`, never committed), adds the OPNsense **`edge→home-PBS:8007`** rule (`sat_ensure_edge_pbs_rule`), and widens the satellite's wg `AllowedIPs` to reach home PBS — nothing else. Client-side encryption key stays home (§3.2). **Not yet live-tested** (no home PBS / live Debian satellite on the test cluster). **Self-patching:** Debian `unattended-upgrades` (security-only + reboot window). **Remaining:** home-side token creation is currently an operator runbook step — could later be automated via `backup-manager` (ADR-007 S9).

---

**Original investigation (2026-07-03) — retained for the record:**
...
- **`:443` — the satellite passthrough is CORRECT; the failure is a test-system Caddy cert issue, not the satellite.** Diagnosed thoroughly: `:443` to these `*.test4.tapaas.org` domains fails with a TLS `internal error` **identically whether reached via the satellite or directly on the LAN (`10.0.0.1:443`)** → Caddy has no valid cert for the test domains (universal, ACME/cert state on the test box). Corrected two earlier mis-reads: it is **not** the access-list (that's a post-TLS HTTP `respond 403` — Caddyfile confirmed; and `identity` has no ACL yet still fails) and **not** a tunnel/bind issue (Caddy binds all interfaces). So the satellite faithfully forwards TLS; full public HTTPS through the satellite will work as soon as Caddy serves a valid cert for a real published domain.
- **Correctness note:** `edge` must **NOT** be added to Caddy's default allow-set the way `netbird` is — `netbird` peers are authenticated admins, but `edge` is the *public, unauthenticated* satellite; defaulting it in would expose internal mgmt services. The model is: services published via the satellite set `proxyAllowedZones: ["internet"]` (no IP restriction), and **PROXY-protocol v2** (§2) is the refinement giving Caddy the real client IP.
- **Remaining (refinement, not a blocker):** PROXY-protocol v2 needs os-caddy to expose a `proxy_protocol` listener-wrapper — the generated Caddyfile has none today (`servers { protocols h1 h2 }`, no listener wrappers), so this is an os-caddy capability to add/verify. Until then the satellite forwards plain (Caddy sees the tunnel IP). Exact `edge` firewall rule form = Q6.
