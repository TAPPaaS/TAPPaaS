...
| **P6** | backup role (PBS **pull** + client-side encryption; immutability via pull model, NOT Object Lock) | TBD | P2, P3, **ADR-007 S9** (backup-manager/controller) | 🟦 unblocked (**Q8→D19**) | Debian satellite path implemented (os selector + `provision-debian.sh`); official `proxmox-backup-server` install + pull-remote body still to build | — | — |
...
**Implemented in this branch (`ADR010-debian-satellite`):** `--os debian|nixos` selector (default `debian`); `satellite/debian/provision-debian.sh` (idempotent, role-gated on-host installer); `lib/provision.sh` renders the Debian configs (`wg-infra.conf`, `nftables.conf`, `nginx-stream.conf`, ip-forward, `unattended-upgrades`) from the same `SAT_*` values + `sat_provision_debian` (SSH deploy, no reformat); schema `os` field; `satellite/debian/README.md`. **Still to build (P6 proper):** the `backup`-role body — official PBS install on the Debian satellite + register home as a pull remote (`--remove-vanished false`, read-only token) + client-side encryption at home. **Self-patching:** Debian `unattended-upgrades` (security-only + reboot window).
