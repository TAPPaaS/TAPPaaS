...
| FW #122 | Reissue **and revoke** tappaas-cicd SSH keys | 3 | 3 | M | Roll in: needed for a cicd rebuild and before #19 can be switched to deny. **Measured on hrossen 2026-09-18:** the cluster-wide `/etc/pve/priv/authorized_keys` holds **15 distinct `tappaas-cicd` keys, one current** — every earlier mothership install still has root on every node, because `distribute_cicd_key` (`cluster/install-platform.sh`) only appends. Also two keys for the departed `tappaas4` and superseded `tappaas2`/`tappaas3` node keys. A reinstalled mothership is locked out of every VM: they got the key once, via cloud-init `sshkeys` at creation. Scope: generate → push to nodes and into running VMs → **revoke the old key**; make `distribute_cicd_key` replace rather than append. The one-off hrossen cleanup was attempted and rolled back (file byte-identical to the original) — to be redone once node-to-node access can be checked the way PVE does it (`HostKeyAlias` + `/etc/pve/nodes/<n>/ssh_known_hosts`) |
...
| #19 | Disable SSH password login on PVE nodes — as a site field | 4 | 2 | M | **Design, operator 2026-09-18:** a site-configurable field `allowRootLogin` in `site.json`, **default allow**, so landing it changes nothing on any site and a test site can keep password SSH. It governs **password SSH to root on the PVE nodes only**: allow = `PermitRootLogin yes` + `PasswordAuthentication yes` (today); deny = `PermitRootLogin prohibit-password` + `PasswordAuthentication no`. Never `PermitRootLogin no`: PVE's cross-node GUI shell and migrations run as root over key SSH (`PVE::API2::Nodes::get_shell_command`, `PVE::SSHInfo::ssh_info_to_command`), and the mothership manages the nodes as root by key. **The consoles are unaffected either way** — web GUI shell is `/bin/login -f root` after the PAM web login, the physical console is getty/PAM, the OPNsense console is its own local login. Does **not** apply to OPNsense: its sshd is already `PasswordAuthentication no`, and a default of allow would loosen it. R drops 4 → 2 because the default is a no-op; switching a site to deny still waits for FW #122. Open: the name — see the issue |
