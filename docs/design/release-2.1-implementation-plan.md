...
| #628 | People → Identity (`people-manager` → `identity-manager`) | 2 | 4 | H | **Gate open (2026-09-18):** ADR-007a is still "People" v1.2 and Erik edits it "once agreed" — the agreement is not on the issue yet. **Measured:** 79 files name `people-manager` (41 in the managers); the nightly sweep calls it by name (`identity/update.sh` `group add` / `user modify`), and so does `site-manager` (`client.ts` `PEOPLE_BIN`) — so binary and callers change in one release, with a `people-manager` alias for one stable cycle. Both sites hold `config/people/` = 1 org, 2 groups, 3 roles, 2 users → a migration (Wave 1, after Wave 0 is on `stable`). **To settle in ADR-007a:** the new directory name — `config/identity/` would sit beside `config/identity.json`, the Authentik module's own config, so `config/identities/` reads better; and User admitting non-human accounts |
