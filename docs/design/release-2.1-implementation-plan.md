...
## 12. Triage — Erik's reports, 2026-09-20/21

Six reports, each checked against `main` before scoring (the *Checked* column says what was
verified, not what the report claims). Scores read as elsewhere: E = ease, R = risk of leaving
it, L = lock-in.

| # | What it is | E | R | Checked on `main` | Do it? |
|---|---|:-:|:-:|---|---|
| #682 | `rules_manager.py` builds `ModuleSpec.location` from the `location` field, which migration 0006 renamed to `moduleSource`.<br>The provider path is then empty, no auto-pinhole compiles, and because `reconcile` prunes, every established pinhole is deleted as an orphan.<br>Erik measured 24 live rules removed across four consumers by a routine sweep. | 4 | 5 | **Confirmed** — `rules_manager.py:402` reads `data.get("location", "")` with no `moduleSource` fallback, against ADR-026 D6.2's "readers accept both for one stable cycle". | **Yes, first.** A one-line read plus the regression test that pins both field names. Connectivity is lost silently, and every site that has run 0006 is exposed |
| #679 | `cloudInit` is `class: immutable` with schema default `true`, so a module that declares nothing still acquires a desired value.<br>A guest that genuinely has no cloud-init drive reports `false`, and the differ calls it immutable drift whose only remedy is delete + reinstall.<br>47 of 48 deployed modules on Erik's estate declare no `cloudInit`, so the exposure is the rule. | 3 | 4 | **Confirmed** — default `true` in `cluster/services/vm/fields.json`, `class: immutable`, and `drift.ts` already records `defaulted` per field. | **Yes.** The fix is a rule, not a patch: a defaulted desired value produces no drift for an immutable field. `defaulted` is already carried, so the change is small and testable |
| #684 | `rules-manager` synthesises auto-pinholes from `dependsOn` **and** `integratesWith` (#632), but `test-module.sh` fills its dependency set from `dependsOn` alone.<br>A service wired through `integratesWith` therefore has a live firewall rule that no test ever verifies.<br>It fails quietly: an unverified rule looks exactly like a healthy one in the report. | 4 | 3 | **Confirmed** — `test-module.sh:223` reads `.dependsOn` only; Steps 2 and 3 both iterate that one variable. | **Yes.** Small, and it closes the blind spot that let #682 delete rules without a single test going red |
| #681 | `repo-sync` stashes local modifications so a pull can proceed, restores the entry it made, and reports how many remain.<br>Nothing lists, ages or resolves the ones left behind, so an operator edit in a managed checkout disappears after one sweep.<br>Erik saw 12 entries in TAPPaaS and 2 in Community; makerfloss carried 16. | 4 | 2 | **Confirmed, with a caveat** — the restore path (#572) only handles the entry that sync created. On makerfloss all 16 were **mode-only** artefacts from before #565 (newest 2026-08-25), so nothing is accumulating now. | **Yes, small.** A listing with age and content summary, and a supported drop/restore; an empty (mode-only) entry can be dropped on sight. Not urgent: the mechanism that produced them is already fixed |
| #680 | Since ADR-017 D3 the mothership rebuilds from the checkout, so `tappaas-cicd/flake.lock` decides every site's nixpkgs.<br>That lock pins 2026-05-22, so a host that pinned something newer moves **backwards** on its first sanctioned rebuild, silently.<br>Erik logged kernel 6.12.92 → 6.12.90, curl 8.20 → 8.19 and six more, with the rebuild reporting success. | 3 | 4 | **Confirmed** — `flake.lock` nixpkgs is `b77b3de8`, 2026-05-22, four months old. | **Yes**, as release process rather than code: the baseline lock is refreshed and reviewed per release, and a rebuild that would lower the revision says so before switching. Needs an owner and a cadence — a §10.2 item |
| #675 | `site.json .repositories` declares the repositories a site tracks and the sweep pulls them all, but the mothership's rebuild only ever builds the baseline flake.<br>A site that composes its own host-level NixOS modules loses them on every scheduled rebuild, while the unit reports success.<br>There is no extension point: `tappaas-cicd.nix` declares no option for it. | 2 | 3 | **Confirmed** — no `mkOption`/`extraModules`/`extraImports` anywhere in `tappaas-cicd.nix`. | **Yes, but design first.** A declared extension point is an architectural commitment (what a managed repository may contribute to the control plane, and what happens when it cannot be built) — an ADR decision, then implementation; not a quick fix |

**Order.** #682 alone, now — it removes live firewall rules. Then #684 with it (the test that
would have caught it), then #679. #681 is a small operator-surface task. #680 is a release-process
decision, #675 an architectural one; both want the operator's word before code.
