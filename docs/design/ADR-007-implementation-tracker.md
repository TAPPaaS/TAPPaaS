...
| **S8** | P8 firewall→network rename + migration (wide surface; slot ≥ S7) | — | S0, S7 | 🟦 steps 1/2/4 done — MODULE renamed firewall→network in source with firewall↔network resolver back-compat (network:* AND firewall:* resolve to whichever config is deployed; stale .location followed; update-tappaas legacy-name map); migration script written (not run); DEPENDENCIES regen. Rules plane (`firewall_manager`/`opnsense-firewall`) + hostname (`firewall.mgmt.internal`/`FIREWALL_FQDN`) KEPT. **Remaining = step 3:** supervised live migration (run migrate-firewall-to-network.sh: rename config/VM/DNS/Caddy + flip vmname/FQDN) with rollback + verify. | module-manager 47/0; fast gate 35/0; live `update-tappaas --dry` EXIT 0 | 3f587ad | ✅ (1/2/4) |
