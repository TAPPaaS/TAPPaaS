...
- **S2b — TODO:** people-manager.ts (CRUD + Authentik sync per the P1 sync semantics: ensure-exists, bidirectional access reconcile, state lifecycle, drift-warn), Nix build (TS pattern from S-TS), identity-controller wiring, sync tests, wire user-setup+sync into 40-Identity install.sh. Then close #56.
