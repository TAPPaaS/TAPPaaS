...
| **S8** | P8 firewall→network rename + migration (wide surface; slot ≥ S7) | — | S0, S7 | ⬜ scoped — **deferred to a focused session (high-risk live migration).** Blast radius 134 files in 3 kinds: (a) MODULE rename 49 files (`firewall:*` deps + firewall.json + src/foundation/firewall/) — do; (b) RULES PLANE 22 files (`firewall_manager`/`opnsense-firewall`) — KEEP; (c) HOSTNAME 34 files (`firewall.mgmt.internal`/`FIREWALL_FQDN`, the cicd's lifeline) — high-risk live VM/DNS/Caddy migration. Stage like the cutover: source rename with firewall:* back-compat aliases → migration script → supervised live migration (rename vmid 110 + DNS + Caddy) with rollback + verify → regen DEPENDENCIES.csv. | — | — | — |
