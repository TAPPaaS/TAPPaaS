...
**All 6 items are now ✅ done** (2026-06-25) — the ADR-007 refactor is feature-complete. What remains are the **documented follow-ups** noted in the cells below (retire the legacy `.sh` once the incremental full-port supersedes them; native TS JSON-schema validation; the deferred health sub-ports; SSO cosmetic binding cleanup; the live switch `--apply`; and the configuration.json-legacy sweep). **Direct-caller migrations done:** ✅ the nightly `update-tappaas` now calls `module-manager module modify <m>` (delegates to update-module.sh — same behaviour, verified via `--dry-run --force`) instead of `update-module.sh` directly; ✅ `backup/install.sh` no longer calls `copy-update-json.sh` directly — it now follows the standard module contract (install-module.sh Step 2 stages/validates/tags the config; the module's install.sh just consumes it), which also fixed a latent tag-loss + override-loss the re-copy caused (backup was the only module install.sh violating the contract). ✅ `cluster/update.sh`'s spurious `validate-configuration.sh` Step-0 **removed** (validated the retired, now-absent configuration.json, warn-only) → `validate-configuration.sh` now has no live runtime caller → **retireable** alongside `create-configuration.sh` once its remaining install-path callers (`install-platform.sh`, `pre-update.sh`, `site-manager/install.sh`) are migrated. #294 (zone-aligned VMIDs) is out of scope, handled separately. Everything else — the S0–S9 build — is in **Done Work** below.
...
| 4 | **Finish the `validate` verb convention** | ✅ **Core done.** All 7 managers now expose a uniform `validate` verb: people (native `validateRefs`) + network (alias of `zones-check`) gained native TS `validate` subcommands; the 5 ported managers wrap their `validate-<x>.sh`. `validate-module.sh` is no longer a stub — it delegates to the native TS `module-manager validate` (the real tier/source lint). people/network README updated; both tsc-clean + test.sh green (people 32+21, network 120+17). **Remaining (follow-ups):** native TS JSON-schema conformance (the 5 + people still defer the deep schema gate to `validate-<x>.sh`); retire the now-redundant people/network `validate.sh` (with the general script retire); create a `manager/TEMPLATE` + P10 contract test encoding the verb contract (TEMPLATE does not exist yet). |
| 5 | **Managers expose CRUD verbs** | ✅ **Done.** people gained full entity CRUD (`org/group/role/user add\|modify\|delete\|list\|show` — validated atomic config writes + reference guards + write-then-reconcile, 26 tests); network `zone get`→`show` (alias kept; no free-form `zone modify` by design — zones' state + access graph are governed by the lifecycle add/delete + zones-init/zones-merge). module/site/environment/backup already had full CRUD; health is read-only (correct). Admins now manage via verbs, never hand-edit JSON. |
| 6 | **#380 — document & revalidate the install sequence** | ✅ **Done.** Revalidated INSTALL.md + INSTALL-ENVIRONMENT.md against the live scripts and fixed the post-cutover staleness: `configuration.json`→ per-env `config/environments/<n>.json` (domain) + `site.json` (email) + `config/cert-refids.json` (TLS refid); `create-configuration.sh --update --domain`→`environment-manager modify <env> --domain`; `firewall/`→`network/` paths (template + zones template now `manager/network-manager/zones.json`, transformed by `zones-init`); doc `create/get`→`add/show`; added the `module-manager add` front-door note. ✅ **The flagged `--dns-mode` gap is now closed:** `environment-manager add/modify` gained `--dns-mode <per-service\|wildcard>` (writes `domains.dnsMode`, enum-validated; INSTALL-ENVIRONMENT + README updated; env tests 21/21) — the wildcard-cert scenario no longer needs a hand-edit. ✅ **Install-flow refactor (A–D):** (A) renamed cicd `install1.sh`→`bootstrap.sh`, `install2.sh`→`install.sh` (tappaas-cicd is now a clean module); (B) split the entry into **`foundation/install.sh`** (orchestrator — the URL the guide downloads; 5-step chain) + **`cluster/install.sh`** (node-only [1/5]; writes `~/tappaas/.cluster-role` for the orchestrator); (C) threaded **`--name <orgname>`** end-to-end so the **Proxmox cluster name = `site.json .name` = default environment = organisation** (was hardcoded `pvecm create TAPPaaS`); (D) rewrote INSTALL.md to the real flow incl. the cicd `install.sh` internals (site/zones/mgmt+default env) and the §3 people/org bootstrap. ⚠️ **NOT yet validated on hardware** — the bootstrap can't be exercised in-repo; verified statically (all `bash -n` clean, `--name` threaded through every hop, no dangling refs). Needs a real fresh-install pass (see the validation checklist handed to the operator). |

> **Out of ADR-007 scope:** *#294 — zone-aligned VMID ranges* (derive/allocate a VM's VMID from its zone/environment) is handled **separately**, not as part of this refactor (operator decision 2026-06-25).
...
|---|------|------------------------|
| ~~F1~~ | **✅ FIXED (working tree) — Unattended install** | (a) `foundation/install.sh` now accepts + forwards `--lan-port`/`--wan-port`/`--pool` (repeatable) → `cluster/install.sh`, which passes them to `config-network.sh` (explicit NIC roles) and `config-storage.sh` (in non-interactive mode it passes `--yes` + the `--pool` specs, and errors loudly if no `--pool` is given since the platform step needs tanka1). (b) `backup/install.sh` no longer hard-prompts: it takes the PBS password from `$TAPPAAS_PBS_PASSWORD`, else prompts on a TTY, else generates one and saves it to `~/.pbs-credentials.txt` (mode 600). Fully-unattended first node: `install.sh "$REPO" ADR007 --name acme --domain acme.org --non-interactive --lan-port nic0 --wan-port nic4 --pool 'tanka1=single:nvme1n1'`. ⚠ NOT yet exercised on hardware (test4 offline) — verified `bash -n` only. |
| ~~F2~~ | **✅ FIXED (working tree) — no `configuration.json` on a fresh install** | `pre-update.sh`'s refresh block now runs `create-configuration.sh --update` **only when `config/configuration.json` already exists** (a legacy, not-yet-migrated system). On a fresh site.json-native install it's skipped, so the vestigial legacy file with the placeholder domain is never minted. (It was the only executing caller in the install/update path; the rest are comments/warnings guarded on the file existing.) ⚠ Not hardware-tested (test4 offline) — `bash -n` only. |
| ~~F3~~ | **✅ FIXED (working tree) — removed the dead `site.json .environments` field** | Confirmed no runtime reader: the TS `--deep` cascade fans out via `SiteClient.listEnvironments()` (the `config/environments/` **directory**), and `config.ts` only *parsed* the field, never used it. Removed: the property from `schemas/site-fields.json` (+ description); the writes in `create-site.sh` and `migrate-configuration.sh`; the `Site.environments` type + its parse in the TS (`types.ts`/`config.ts`); the two `test.sh`/`test-create-site.sh` length assertions (now assert the field is *absent*). Environments are simply whatever is under `config/environments/`. ⚠ TS not typechecked locally (no tsc) + not hardware-tested. |
| ~~F4~~ | **✅ FIXED (working tree) — Step-6 failures are now diagnosable** | `install-module.sh` Step 6 now `tee`s the module installer's combined output to `/tmp/tappaas-install-<m>.XXXXXX` while streaming it live, and on failure replays the last 20 lines + points at the saved log before `die` (pipefail makes the `if` observe `install.sh`'s status, not `tee`'s). No more bare *"Module install.sh failed"* with no cause. |
| ~~F5~~ | **✅ MITIGATED (working tree) — logging first-install hardening** | ⚠ The exact Step-6 failure was never captured (it was swallowed — that's F4), and `identity/update.sh` has the *same* top-of-file pattern yet installed fine, so the original cause is **unconfirmed**. What was done: (a) **F4** guarantees the next occurrence is visible; (b) defensive hardening — `logging/update.sh` (and `identity/update.sh`) read the **display-only** `vmid` with a default instead of aborting under `set -e` if it isn't written back to the module config yet on the first pass. NOTE: `logging/update.sh` does NOT touch the VM's Loki/Grafana — it only wires syslog forwarding (already best-effort/`warn`), so the earlier "wait for services" framing was inaccurate; the syslog wiring reconciles on the next `update-tappaas` regardless. If it recurs, F4's saved log will show the real line. |

**Retest tooling (new):** `src/foundation/uninstall.sh` — the counterpart to
`foundation/install.sh`. Tears a node back toward a verbatim Proxmox install
(destroys TAPPaaS-tagged VMs, PBS, `tank*` pools, dissolves the cluster, restores
the pre-TAPPaaS `/etc/network/interfaces`, removes the tappaas user + markers).
**Dry-run by default**; `--yes` to execute (typed `ERASE` confirm on a TTY);
`--keep-pools`/`--keep-network`/`--keep-user`/`--only <steps>` escape hatches. ⚠
Coded against the observed test4 layout but NOT yet run on hardware (test4
offline) — verify the cluster-dissolve + network-restore steps on the next box
...
| D4 | **Organization has no identity-side representation (potential fix)** | TAPPaaS **organizations are a config-only ownership construct** — `people-manager reconcile` syncs roles/groups/users to Authentik but **NOT the org** (verified 2026-07-03: `reconcile.ts` has no org action; live dry-run showed 13 actions, none for the org). So `config/people/organizations/<name>.json` + `site.json` `owner`/`organizations[]` exist, but Authentik has no object for the org itself — only its people. **Potential fix:** map a TAPPaaS org to an Authentik-side representation (e.g. a top-level Authentik **group** `<org>` that the org's user-groups nest under, or an Authentik **brand/tenant**), added as an `ensure-org`→group action in the people reconcile. Design decision (which Authentik primitive; single- vs multi-tenant) — deferred; the config/site linkage works without it. |
...
| **Revisit the `cluster/services/vm/` service-hook implementation** (`install-service.sh` + `update-service.sh`) | These hooks re-invoke the install *primitives* directly — `install-service.sh` lazily auto-builds a missing base-image template via `install-module.sh <template-module>` (`install-module(app) → vm-service → install-module(template)`); `update-service.sh` shells to `update-os.sh`. This is the **correct layering** (implementation hooks call the `install-module.sh`/`update-os.sh` primitive, NOT the `module-manager` operator front door — so they are *not* front-door "holdouts"). The re-entry is guarded (only `autoInstall:true` templates auto-build; the nixos/winserver templates are `autoInstall:false` → die-with-manual-instructions, so the auto-build path is currently **dormant**) and `templates` is already a foundation prerequisite, making the lazy build a fallback. **To revisit:** (a) decide whether the lazy auto-build earns its keep vs. making `templates` a hard prerequisite and removing the re-entry entirely; (b) tidy/clarify the hook↔primitive contract so the layering is explicit (and the audit's "direct caller" framing isn't mistaken for a holdout); (c) review the `autoInstall` capability now that no template uses it. Architectural review, not a compat-shim removal — low urgency. |
| **Revisit the bootstrap (`install2.sh`) direct-script calls** | `install2.sh` calls the install primitives directly — `create-site.sh`, `create-minimal-environments.sh`, `copy-update-json.sh`, and `update-module.sh` (cluster/templates/network/tappaas-cicd). This is a **chicken-and-egg**: `install2.sh` is the script that *builds and links the managers themselves* (module/site/environment/network/…), so the early steps run **before** those bins exist on PATH and cannot go through `module-manager`/`site-manager`/etc. So most of these are **inherent to bootstrap**, not holdouts. **To revisit:** (a) identify which `install2.sh` steps run *after* the managers are built+linked and could/should switch to the verb front door (e.g. the later `update-module.sh` / `create-minimal-environments.sh` calls), vs. which are genuinely pre-manager and must stay primitive; (b) consider a clean two-phase split in `install2.sh` (pre-manager primitive bring-up → managers linked → manager-verb-driven remainder) so the boundary is explicit; (c) confirm `copy-update-json.sh` stays a primitive (no front-door equivalent — it's the staging step `install-module.sh` itself uses). Architectural review, low urgency. |
