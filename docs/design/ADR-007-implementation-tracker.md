...
| **S0** | P4-structure: manager/controller skeleton + script moves + dispatchers | #365, #364 (layout) | none | 🧪 batch 3 (driver, in progress) | B1 cicd: variant 6/0 (VM-suite env); B2 controllers 107/1 (1 pre-existing ap WIP); firewall --deep = baseline (no new fails) | (WIP) | 🟦 |
