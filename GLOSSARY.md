...
| **Composition** | *how is it built?* | ADR-009 |
...
| **Controller** | A leaf control-plane component inside a Module that operates one specific function (e.g. `opnsense-controller`, `switch-controller`). Invoked by the Manager; not exposed to end-users directly. Invokes the Module's Service scripts internally (today: .sh; roadmap: Python wrapping .sh). Corresponds to a Function in ArchiMate terms. |
