...
| **zone0 / zone1** | The security zone each network interface connects to (must exist in `zones.json`). For a VM or LXC: the Proxmox bridge VLAN of its virtual NIC. For a machine: the network its physical NIC is cabled to — recorded, not yet enforced on the switch. |
| **address** | How the mothership reaches a machine: an IP or FQDN, logged into as root with the mothership's key (ADR-026 D8.1). A VM or LXC is reached by name and zone instead. |
...
| **os** | A facet of every system (`machine`, `vm`, `lxc`), in two levels (ADR-022f D7): `os.family` = `linux` \| `windows`; `os.id` = `debian`, `nixos`, … An `application` inherits its Host's OS; a `device` has none. Held as one field, `os`, whose value is the `id`; the family is derived (`debian`/`ubuntu`/`nixos` → `linux`). |
